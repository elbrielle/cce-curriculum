...
15. **Teacher-facing prose discipline: no em dashes in `docs/` body prose; admonitions match the Wk0 length profile.** The first-pass 6SW Wk6 admonition (commit `01627b1`) was accepted as structurally correct but the user flagged it on the live site as "too long" and flagged two em dashes ("Adapt to your end-of-year schedule — everything here is optional" and "This is the capstone pass — not the artifact week") as disliked. Commit `7ccdce2` tightened both: admonition went from ~15 to ~11 visible lines by cutting redundant sentences, dropping verbose "primary home is" phrasing in the bullet list, and shortening enumerated parentheticals; em dashes replaced with commas or removed entirely. Rule going forward, for ALL teacher-facing content in `docs/` (NOT for PLANNING.md, CLAUDE.md, notes files — those are developer-facing and em dashes are fine): **(a) draft the admonition, then cut 20% before shipping; target ≤12 visible lines to match the 1SW Wk0 `!!! abstract`;** **(b) before committing, grep your diff for em dashes (`git diff | grep -- "—"`) and replace with commas / periods / restructured sentences;** **(c) avoid redundant "X, not Y" rhetorical constructions after a "nothing is critical" declaration;** **(d) the rule applies to the admonition itself AND to any body claims softened to match it — the softening should be 1-2 sentences, not a section rewrite.** Saved to memory as `feedback_prose_style.md`.
