...
- **Canvas image first-load performance audit (P2).** Teachers reported that images in the first six weeks can be slow on first load. The 2026-08-08 1SW Weeks 0-5 local Canvas-only baseline contains 71 PNG images totaling 35.1 MB; 33 exceed 500 KB and those files alone total 26.0 MB. It also contains three exact-duplicate groups (7, 5, and 4 copies). Pilot a visually lossless-enough optimization pass on the Week 1 Day 1 full-page opener and the seven-image Week 5 Day 2 email lesson before changing Canvas: preserve readable text at Chromebook/iPad widths, compare the original and optimized image at 100% and mobile width, record combined page weight and cold-load behavior, retain descriptive alt text, and replace only when instructional detail is unchanged. Reuse one locked Canvas file for exact duplicates where practical. Do not lower resolution by a blanket batch rule or recompress licensed originals destructively. The whole-course baseline remains 162 raster images totaling 76.8 MB.
