---
title: Skilled-Trades Current Entry Routes
slug: 5sw-wk4-current-entry-routes
kind: worksheet
weeks: 5sw/wk4-hvac-electrical-plumbing
audience: student
variant_of:
language: en
pages: 4
orientation: portrait
---

## Trade-Specific State Boundaries

One ladder does not fit every skilled trade.

| Trade | Current Texas boundary to verify |
|---|---|
| Electrical | TDLR license type, experience hours, education, application, and exam eligibility |
| Plumbing | TSBPE apprentice registration, required hours/course or approved program, application, and exam |
| HVAC | TDLR technician registration/certification under a licensed contractor; contractor licensure is separate |
| Welding | Training and employer/process credentials vary; do not invent one universal Texas license |

**Word bank / Banco de palabras:** eligibility/requisitos · enrollment/inscripción · apprenticeship/aprendizaje registrado · estimate/estimación · verify/verificar

One important difference among these boundaries:

[[lines: 5]]

One official agency and the question I would verify there:

[[lines: 5]]

[[pagebreak]]

## Two Current Route Cards

### Registered Apprenticeship

A Registered Apprenticeship is a paid job with structured on-the-job learning, related instruction, mentoring, progressive wages, and a portable industry-recognized credential.

Sponsor-specific details that must be checked: current opening, occupation, eligibility, application, screening, length, wage progression, schedule, cost, credential, and license relationship.

Future verification sequence:

1. Search the Apprenticeship Job Finder by occupation and location.
2. Open a current employer or sponsor listing.
3. Record the listing's eligibility, documents, training, wage, credential, location, and deadline.
4. Apply directly through the listed employer or sponsor when eligible and ready.
5. Complete the sponsor's stated screening and next steps.

### Dallas College Electrical Technology

Current published examples updated June 25, 2026 include:

- Electrical Pre-Apprentice Continuing Education certificate: estimated 6 months and $5,184 in-county.
- Commercial Electrical credit certificate: estimated 8 months and $2,673 in-county.

These are published estimates, not promises. Cost, time, prerequisites, campus availability, aid, and transfer treatment must be checked for the student's cycle.

Future verification sequence:

1. Choose the current credit or continuing-education route.
2. Check campus, schedule, prerequisites, cost basis, credit status, aid, and transfer limits.
3. Complete the appropriate Dallas College application when ready.
4. Complete required advising or readiness steps.
5. Register for the correct current courses with college guidance.

One sponsor condition and one college condition that could change:

[[lines: 4]]

[[pagebreak]]

## Compare Two Routes

| Evidence | Registered Apprenticeship | Dallas College route |
|---|---|---|
| How to start |  |  |
| Paid work during route |  |  |
| Published cost or variable |  |  |
| Credential or outcome |  |  |
| One condition to verify |  |  |

Best route for fictional Jordan and two card details supporting the decision:

[[lines: 7]]

One advantage of the route Jordan did not choose:

[[lines: 4]]

**Complete frame:** I recommend [route] for Jordan because [card detail 1] and [card detail 2]. The other route may be stronger when [advantage/condition].

[[pagebreak]]

## Ordered Next Steps

Selected route: ____________________________________________

1. First exact action:

[[lines: 3]]

2. Next exact action:

[[lines: 3]]

3. Then:

[[lines: 3]]

One variable condition that must be rechecked:

[[lines: 4]]

Official source and why it is the right source:

[[lines: 4]]
