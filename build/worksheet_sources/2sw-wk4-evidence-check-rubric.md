---
title: Health Career Evidence Check Rubric
slug: 2sw-wk4-evidence-check-rubric
kind: rubric
weeks: 2sw/wk4-dental-medical-billing
audience: teacher
variant_of:
language: en
pages: 2
orientation: landscape
---

## Recommended minor checkpoint: 16 points

| Criterion | Masters - 4 | Meets - 3 | Approaches - 2 | Needs Improvement - 1 | No evidence - 0 |
|---|---|---|---|---|---|
| Career data accuracy | All 3 careers accurate; pay keeps year, geography, measure | One minor error | Partial table or mixed labels | Major errors | Blank/off topic |
| Classification | All 3 use the stated comparison rule and evidence | One classification needs detail | Rule used inconsistently | Labels without evidence | Blank/off topic |
| Scenario fit | Clear recommendation connects preparation and duties to Jordan | Clear fit with one missing detail | General fit with one fact | Preference only | Blank/off topic |
| Evidence and trade-off | 2+ accurate facts plus a meaningful trade-off/question | 2 accurate facts with basic explanation | 1 accurate fact or thin reasoning | Unsupported claim | Blank/off topic |

## Score record

| Criterion | Points |
|---|---:|
| Career data accuracy | /4 |
| Classification | /4 |
| Scenario fit | /4 |
| Evidence and trade-off | /4 |
| **Total** | **/16** |

## Performance-band conversion

| Raw points | Percent range | Band |
|---:|---:|---|
| 15-16 | 90-100 | Masters |
| 13-14 | 80-89 | Meets |
| 12 | 70-79 | Approaches |
| 10-11 | 60-69 | Needs Improvement |
| 0-9 | Below 60 | Follow campus/district policy |

## Calibration notes

- Do not deduct for handwriting, platform route, artistic quality, or English mechanics unless meaning is unclear.
- A Dental Hygienist recommendation can earn full credit even though it exceeds Jordan's stated preference if the student names that trade-off and explains why other evidence outweighs it.
- Medical Records Specialist has no verified current Irving program connection. Do not require students to invent one.
- Dental Assistant is below the course high-wage line but above the growth line. One label does not decide overall career value.

Strength: ___________________________________________________________________

Fact to correct: _____________________________________________________________

Next revision: ______________________________________________________________
