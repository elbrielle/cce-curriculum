...
async def upload(c, path, folder):
    ...
    if not record.get("locked"):
        record = await api(c, "PUT", f"/files/{record['id']}", data={"locked": "true"})
    return record
...
async def main():
  ...
  <ol>
    ...
    <li>Name one transferable communication skill and explain what the skill looks like in two careers.</li>
  ...
  <p><strong>Submit:</strong> Type the four parts here, upload a document, or record a brief media response. Upload the paper route only when your teacher assigned it.</p>
        ...
        student = {
            1: {
                ...
                "SHOW_LEARNING": "Complete the FYF conflict plan and a two-career transfer response.",
                ...
                + step(
                    ...
                    + "<p>Name what each person needs, one specific solution, and how the team will know it is fair.</p>",
                )
                + step(
                    4,
                    "Transfer the skill",
                    "<p>Name two careers, a conflict each could face, and the first safe move.</p>",
                ),
                ...
                "SUPPORT": "<p>conflict = conflicto · listen = escuchar · respectful = respetuoso. Frame: “Each person needs ____. We can ____ so that ____.”</p>",
            ...
            2: {
                ...
                "SHOW_LEARNING": "Complete FYF p. 63, one safe practice note, and the transfer check.",
                ...
                + step(
                    ...
                    + "<p>Record two essential and two background details with reasons.</p>",
                )
                + step(
                    3,
                    "Ask two new questions",
                    "<p>Do not repeat information already in the account.</p>",
                )
                + step(
                    4,
                    "Try one safe workplace card",
                    "<p>Paraphrase, ask one question, and name a safe next step. Acting and written analysis are equal.</p>",
                )
                + step(
                    5,
                    "Use the feedback quiz",
                    f'<p><a href="{quiz_url}">Open the Active Listening Evidence Check</a>. Retry and read the feedback.</p>',
                ),
                "DONE": "<ul><li>four details sorted with reasons;</li><li>two new questions;</li><li>one safe practice note;</li><li>one transfer example.</li></ul>",
                "SUPPORT": "<p>essential = esencial · background = contexto · paraphrase = parafrasear · clarify = aclarar.</p>",
            ...
            3: {
                ...
                + step(
                    ...
                    "<p>Use a real course checkpoint. Your goal needs an action, measure, reason, and deadline.</p>",
                )
                + step(
                    3,
                    "Protect the time",
                    "<p>Schedule two short work blocks. Name an obstacle and an if-then backup.</p>",
                )
                + step(
                    4,
                ...
                "SUPPORT": "<p>goal = meta · deadline = fecha límite · obstacle = obstáculo · backup = alternativa. Frame: “By ____, I will ____ as shown by ____.”</p>",
            ...
            4: {
                ...
                "READY": f"<p>Open your workbook to FYF pp. 147-148. Use {file_link(files['WRITE']['id'], 'the optional Written Message Lab')} only when your teacher assigns the no-workbook or extended-rewrite route.</p>",
                "STEPS": step(
                    1,
                ...
                + step(
                    ...
                    f'<p><a href="{discussion_url}">Open the optional Little Library Message Lab discussion</a>, or use the private written route. Both use Notice + Question + Next Step.</p>',
                )
                + step(
                    4,
                    "Rewrite one fixed message",
                    "<p>Use only the supplied facts. Do not invent medical guidance, test results, charting, or workplace policy.</p>",
                ),
                ...
                "SUPPORT": "<p>status = estado · audience = audiencia · concise = conciso · proofread = revisar.</p>",
            ...
            5: {
                ...
                + step(
                    ...
                    '<p><a href="https://www.careeronestop.org/Toolkit/Skills/skills-matcher.aspx">Open the CareerOneStop Skills Matcher</a>. Rate all 40 areas. Record two suggestions and one pattern. Do not submit a screenshot.</p>',
                )
                + step(
                    3,
                    "Revise the plan",
                    "<p>Update the SMART goal, time block, and backup strategy from Day 3.</p>",
                )
                + step(
                    4,
                    "Show transfer",
                    "<p>Compare one Powerskill across two careers. Name what stays the same and what changes.</p>",
                ),
        ...
        teacher = {
            1: {
                ...
                "TEKS": "d(4)(B)",
                "DOL": "FYF Conflict Resolution Plan and two-career transfer response.",
                ...
                "PREP": f"<ul><li>Open the embedded FYF pages and have students use FYF pp. 144-145.</li><li>Keep {file_link(files['CONFLICT']['id'], 'the optional no-workbook route')} available instead of printing it for every student.</li><li>Project or post the {file_link(files['GUIDE']['id'], 'transfer guide')}.</li><li>Assign reader, recorder, facilitator, and optional designer roles.</li></ul>",
                ...
                + flow(
                    "#1f617a",
                    "Debrief and exit · 10",
                    "Compare consequences and transfer to two careers.",
                ),
                "MONITOR": "<p>Strong solutions name each need, a fair action, and a backup. Voting alone is incomplete. If safety is involved, protect safety and notify the appropriate supervisor; do not compromise the rule.</p>",
                "SUPPORT": "<p>Model the Name row. Permit independent, speech-to-text, and oral-rehearsal routes. Do not grade drawing.</p>",
                "FALLBACK": "<p>The student page contains all sources. An absent student completes the same plan independently.</p>",
            },
            2: {
                "TITLE": "Listen for the Detail That Matters",
                "TOPIC": "Transferable Skills",
                ...
                "TEKS": "d(4)(B)",
                "DOL": "FYF p. 63 responses, safe workplace practice note, and transfer check.",
                "SUBTITLE": "50 minutes · TEKS d(4)(B)",
                ...
                "PREP": f"<ul><li>Open the embedded Maria account and response page; students use FYF pp. 62-63.</li><li>Keep {file_link(files['LISTEN']['id'], 'the optional Active Listening Lab')} for the no-workbook or extended-practice route.</li><li>Open the unpublished practice quiz.</li></ul>",
                "EVIDENCE": "<p>Four justified details, two new questions, one safe practice note, and a transfer example.</p>",
                "FLOW": flow(
                    "#5a2d91",
                    "Warm-up · 5",
                    "Name a strategy for a detail-heavy story.",
                )
                + flow("#4a9d2f", "Two reads · 8", "Pencils down, then mark evidence.")
                ...
                + flow(
                    "#1f617a",
                    "Safe practice and quiz · 16",
                    "Paraphrase, question, route, retry feedback.",
                ),
                "MONITOR": "<p>Essential: persistent chest tightness, onset/time, shortness of breath, spread to shoulder, blood-pressure medication/history, family heart history. Background: paperwork, busy week, bag/coffee unless well defended. Do not accept a diagnosis as listening evidence.</p>",
                "SUPPORT": "<p>Keep the written account visible. Offer actor, listener, observer, and written roles. Pre-teach essential, background, paraphrase, and clarify.</p>",
                "FALLBACK": "<p>The embedded pages and optional lab are complete without the quiz. No student shares personal health information or performs publicly.</p>",
            },
            3: {
                "TITLE": "Advocate, Set a Goal, and Protect the Time",
                "TOPIC": "Goals and Time",
                ...
                "PREP": f"<ul><li>Open FYF p. 134; keep p. 135 optional.</li><li>Post {file_link(files['SMART']['id'], 'the SMART/time plan')} and {file_link(files['RUBRIC']['id'], 'weekly rubric')}.</li><li>Provide a real course checkpoint or date.</li></ul>",
                ...
                + flow(
                    "#1f617a", "SMART model · 8", "Action, measure, reason, deadline."
                )
                + flow(
                    "#e3ad19",
                ...
                "MONITOR": "<p>Reject “someday,” impossible platform counts, and deadlines unrelated to the current block. Full evidence includes an action under the student's control and a backup that changes the route, not the goal.</p>",
                "SUPPORT": "<p>Use the full sentence frame. Goals remain private by default. A fictional goal may replace personal disclosure.</p>",
                "FALLBACK": "<p>No platform is required. Speech-to-text, teacher conference, and private self-check are valid.</p>",
            },
            4: {
                "TITLE": "Write So the Reader Can Act",
                "TOPIC": "Transferable Skills",
                ...
                "TEKS": "d(4)(B)",
                "DOL": "FYF Little Library message, feedback or self-check, and fixed-fact workplace rewrite.",
                "SUBTITLE": "50 minutes · TEKS d(4)(B)",
                ...
                "PREP": f"<ul><li>Open the embedded FYF pages; students use FYF pp. 147-148.</li><li>Keep {file_link(files['WRITE']['id'], 'the optional Written Message Lab')} for the no-workbook or extended-rewrite route.</li><li>Choose Discussion or private route before class.</li></ul>",
                "EVIDENCE": "<p>Fictional Little Library post, feedback/self-check, fixed-fact workplace rewrite, and transfer check.</p>",
                "FLOW": flow(
                    "#5a2d91",
                    "Missing-detail warm-up · 5",
                    "Identify what the reader still needs.",
                )
                + flow(
                    "#4a9d2f",
                    "Four writing checks · 8",
                    "Reader, clarity, topic, proofread.",
                )
                + flow(
                    "#1f617a",
                    "Little Library post · 20",
                    "Fictional status, action, and two hashtags.",
                )
                + flow(
                    "#e3ad19",
                    "Fixed-fact rewrite · 12",
                    "Use only supplied facts and safe routing.",
                )
                + flow(
                    "#1f617a",
                    "Transfer exit · 5",
                    "Compare readers across two careers.",
                ),
                "MONITOR": "<p>Look for status, audience, action, and safe routing. No real location/contact details. Do not accept invented medical guidance, test-result interpretation, charting, or policy.</p>",
                "SUPPORT": "<p>Offer typed, handwritten, or short audio drafts. Use Notice + Question + Next Step. Do not grade hashtags or grammar unless meaning is unclear.</p>",
                "FALLBACK": "<p>Skip public posting for privacy, absence, or accommodation. The workbook plus embedded fixed-fact prompt is the complete route; the optional lab is the paper fallback.</p>",
            },
            5: {
                "TITLE": "Record Experience and Connect Skills to a Plan",
                "TOPIC": "Goals and Time",
                ...
                "PREP": f'<ul><li>Test ClassLink, Xello, and CareerOneStop on a student Chromebook.</li><li>Open Xello Completion Standards.</li><li>Open the unpublished <a href="{minor_url}">Communication and Goal Synthesis</a>, {file_link(files["RUBRIC"]["id"], "rubric")}, and the licensed {file_link(files["XELLO"]["id"], "My experiences facilitator guide")}.</li><li>Use {file_link(files["SYNTH"]["id"], "the two-page synthesis")} only for the assigned paper route.</li></ul>',
                "EVIDENCE": "<p>Required Xello save/report plus the four-part Canvas Minor: revised goal, time plan and backup, two-career skill transfer, and one evidence-based next action.</p>",
                "FLOW": flow(
                    "#5a2d91",
                    "Responsibility warm-up · 5",
                ...
                + flow(
                    "#1f617a",
                    "Skills Matcher · 20",
                    "Complete 40 ratings; record two suggestions and one pattern.",
                )
                + flow(
                    "#e3ad19",
                    "Private synthesis · 12",
                ...
                "MONITOR": "<p>The district minimum is one Work experience. The broader Xello guide supports a 10-15 minute lesson and may ask for three experiences; label it as extended support. Do not require screenshots or public result sharing. Results are suggestions from self-ratings.</p>",
                "SUPPORT": "<p>Read the Skills Matcher anchors aloud in chunks. Offer a private written or teacher-conference analysis. Use the same transfer criterion with sample careers if the site is blocked.</p>",
                "FALLBACK": "<p>Paper does not replace Xello completion. Move the save to supervised catch-up. Time Management and any Xello video are optional extensions only.</p>",
            },
        }
        titles = {
        ...
        for position, (kind, key, title) in enumerate(order, start=1):
            item = next(
                ...
                or (
                    kind in ("Quiz", "Discussion", "Assignment")
                    and i.get("content_id") == key
                )
            )
            await api(
                c,
