    ...
    async with httpx.AsyncClient(headers={"Authorization":f"Bearer {token}"},timeout=120) as c:
        ...
        teacher_data={
          1:{
            ...
            "FLOW":(
              flow("#5a2d91","Minutes 0-5 - Welcome and digital risk warm-up",'''<p>Welcome students and project the cybersecurity launch prompt.</p><ul><li>Ask, <em>“A school account is sending messages that its owner did not write. What needs to happen FIRST: investigate the account and revoke access, redesign the school's public website, or buy every student a new laptop? Why?”</em></li><li>Collect two quick responses. Emphasize: investigate the breach and protect access immediately before spending money or altering unrelated systems.</li><li>Bridge with, <em>“Today we investigate the professionals whose job is protecting critical digital systems from unauthorized access.”</em></li></ul>''')+
              flow("#4a9d2f","Minutes 5-15 - District context and critical source labels",'''<p>Open FYF pp. 36-38 to locate Irving ISD's Cybersecurity program of study.</p><ul><li>Distinguish three essential terms: (1) a <strong>program of study</strong> is a multi-year high-school sequence; (2) an <strong>occupation</strong> is a real job workers are paid to do; and (3) an <strong>industry credential</strong> proves a specific skill standard, not an employment guarantee.</li><li>Project the Cybersecurity Career Route Guide and model the source labels: May 2024 U.S. median ($124,910), 2024-34 projected growth (29%), and typical preparation (bachelor's degree + related experience).</li><li>Contrast: national median is NOT a local starting salary guarantee; 29% growth is a statistical projection, not an automatic job offer.</li></ul>''')+
              flow("#1f617a","Minutes 15-40 - Three-role comparative analysis and route building",'''<p>Students read the three progressive role cards on the Route Guide.</p><ul><li>Role 1: <strong>Computer User Support Specialist</strong> (entry foundation).</li><li>Role 2: <strong>Network and Computer Systems Administrator</strong> (infrastructure security).</li><li>Role 3: <strong>Information Security Analyst</strong> (specialized cybersecurity defense).</li><li>Students complete the five-step route builder and highlight: (1) one core duty that shifts across roles, (2) typical entry preparation, (3) national median pay and growth projection, and (4) one concrete source limitation.</li><li>Conduct a Stop and Jot: <em>“This source helps me understand [trend], but it does not tell me [unanswered question/local reality].”</em></li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 20): Verify students are recording exact BLS figures with their May 2024 labels.<br>• Lap 2 (Minute 28): Check that students identify a valid source limitation (e.g. national vs. DFW, median vs. entry pay).<br>• Lap 3 (Minute 35): Intervene with students treating the three roles as a mandatory locked ladder rather than one possible career pathway.</li></ul>''')+
              flow("#e3ad19","Minutes 40-47 - Emerging occupation evaluation DOL",'''<p>Direct students to the Day 1 Exit Evaluation.</p><ul><li>Prompt 1: Explain why Information Security Analyst is classified as an emerging career (29% projected growth vs. 3% national average).</li><li>Prompt 2: State typical entry preparation and one primary job duty.</li><li>Prompt 3: Explain one limitation of the data and formulate one unanswered research question for further investigation.</li></ul>''')+
              flow("#24323d","Minutes 47-50 - Collect route guides, save, and close",'''<p>Collect materials and secure student records for Day 4's revision.</p><ul><li>Collect the Cybersecurity Career Route Guides—these will be returned on Day 4.</li><li>Ensure Chromebooks are docked.</li><li><strong>Safe Trim:</strong> At minute 40, stop reading and transition all students to the evaluation exit. Omit optional CyberSeek browsing; protect the three-role route builder and written evaluation.</li></ul>''')
            ),
          ...
          2:{
            ...
            "PREP":f'<ul><li>Print/post the <a href="/courses/{COURSE_ID}/files/{files["CHECK"]["id"]}/preview">Phishing Red-Flag Checklist</a>.</li><li>Open the seven authenticated Canvas email images and FYF pp. 24-25.</li><li>Post the response: Pause, verify independently, report, delete.</li></ul>',
            ...
            "FLOW":(
              ...
              flow("#1f617a","Minutes 15-38 - Seven-email structured investigation",'''<p>Open the seven authenticated Canvas email images in sequence.</p><ul><li>Students inspect each email and record evidence on their Phishing Red-Flag Checklist before making a call: Safe-looking vs. Spoofed.</li><li>Run Think-Pair-Share after Email 4: <em>“The strongest red flag is [clue] because [reason]. The safe professional response is [action].”</em></li><li>Remind students: 'Safe-looking' (Emails 3 &amp; 7) does not mean proven safe. In a real workplace, unexpected corporate emails are verified through official internal portals or known HR contacts.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 20): Check Email 2—verify students catch the spoofed spelling ('amaz0n' with a zero).<br>• Lap 2 (Minute 28): Check Email 5—verify students identify the false one-hour deadline and unrelated URL.<br>• Lap 3 (Minute 34): Confirm students evaluate visible evidence rather than guessing.</li></ul>''')+
              flow("#e3ad19","Minutes 38-46 - Professional response and integrity DOL",'''<p>Direct students to the Day 2 Exit Prompts.</p><ul><li>Prompt 1: Which of the seven emails was the hardest call, and what visible evidence decided your choice?</li><li>Prompt 2: Explain why you verify through an independent channel rather than replying or clicking a link inside the message.</li><li>Prompt 3: How does a cybersecurity professional demonstrate work ethic and integrity when handling user data?</li></ul>''')+
              flow("#24323d","Minutes 46-50 - Safe practice review and close",'''<p>Review findings and conclude the investigation.</p><ul><li>Collect checklists and review the 4-step protocol aloud with the room.</li><li>Dock Chromebooks properly.</li><li><strong>Safe Trim:</strong> If student email analysis runs long, analyze five emails instead of seven (protecting Emails 1, 2, 3, 5, and 6). Never cut the independent verification explanation or integrity reflection.</li></ul>''')
            ),
            ...
            "FALLBACK":"<p>All seven images and the checklist are in the student guide for absences. On touch devices, use visible domains and independent verification; do not require long-pressing an unknown link.</p>"},
          3:{
