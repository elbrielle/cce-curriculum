...
async def main():
    ...
    async with httpx.AsyncClient(headers={"Authorization":f"Bearer {token}"},timeout=120) as c:
        ...
        teacher_data={
          ...
          2:{"TITLE":"Programming Pathway Deep-Dive","SUBTITLE":"50 minutes - TEKS d(1)(C), d(2)(A)","ALERT":"<strong>Verify the four named Hats live.</strong> If App or Game Developer does not map to its own BLS page, use the broader Software Developers category and label that choice.","PREP":f'<ul><li>Open H&amp;L IT &gt; Programming and Software Development.</li><li>Pre-open BLS Software Developers and Web Developers.</li><li>Print the <a href="/courses/{COURSE_ID}/files/{files["RESEARCH"]["id"]}/preview">career sheet</a>, optional <a href="/courses/{COURSE_ID}/files/{files["EXAMPLE"]["id"]}/preview">model</a>, and <a href="/courses/{COURSE_ID}/files/{files["E2"]["id"]}/preview">exit ticket</a>.</li></ul>',"EVIDENCE":"<p>Formative/minor option: four-row Hat table plus the six-field career sheet using two named sources. The mobile-game exit is formative.</p>","FLOW":flow("#5a2d91","Warm-up - 5 minutes","Sort familiar apps/sites/games by who builds them.")+flow("#4a9d2f","Four-Hat comparison - 25 minutes","Students record education, local pay, demand, and one reaction for each role.")+flow("#1f617a","One-career research - 15 minutes","Complete six fields and confirm pay/training with BLS.")+flow("#e3ad19","Hiring case - 5 minutes","Choose App or Game Developer and name a cross-platform tool such as Unity/C# or a cross-platform app framework."),"MONITOR":"<p>Every row needs a career, education, local salary, demand, and reaction. Accept App Developer with a cross-platform framework or Game Developer using Unity/C# for the mobile-game case when the reasoning addresses iOS and Android.</p>","SUPPORT":"<p>Use the Web Developer model, bilingual field labels, and oral rehearsal before writing. Keep platform vocabulary on the board.</p>","FALLBACK":"<p>Use the worked model and BLS pages for one complete career sheet and exit ticket. Rebuild the four H&amp;L rows in catch-up before Day 3.</p>"},
          3:{"TITLE":"Powerskill Resilience + IT Salary Showdown","SUBTITLE":"50 minutes - TEKS d(1)(C), d(5)(A), d(5)(E)","ALERT":"<strong>Use dated sources.</strong> Xello is the default local salary source; H&amp;L may replace it only after the teacher verifies the displayed DFW range. BLS provides national figures.","PREP":f'<ul><li>Open FYF pp. 26-27, Xello career salary pages, and three clean BLS occupations.</li><li>Record retrieval dates and current figures on a teacher key.</li><li>Print the <a href="/courses/{COURSE_ID}/files/{files["SALARY"]["id"]}/preview">salary sheet</a>, <a href="/courses/{COURSE_ID}/files/{files["GUIDE"]["id"]}/preview">BLS guide</a>, and <a href="/courses/{COURSE_ID}/files/{files["E3"]["id"]}/preview">exit ticket</a>.</li></ul>',"EVIDENCE":"<p>The salary sheet plus Day 5 Career Fit Reflection is a 20-point <strong>major grade</strong>. Today, collect and return the sheet for the reflection. Flip the Failure and the exit matrix are formative.</p>","FLOW":flow("#5a2d91","Warm-up - 5 minutes","Name a setback and a specific next move.")+flow("#4a9d2f","Flip the Failure - 15 minutes","Four failures, each with one technical fix and one resilience action.")+flow("#1f617a","Salary comparison - 25 minutes","Chunk local salary, BLS pay, education, outlook, and job count; use a teacher source for DFW employers.")+flow("#e3ad19","Evidence claim - 5 minutes","Choose the best salary/growth combination and cite a number."),"MONITOR":"<p>Strong fixes: alert threshold/scheduled test/owner; update and MFA/patch owner; second reviewer/escalation rule; incident roles and stop authority. Pivot if students confuse annual/hourly pay, local range/national median, or outlook percent/job count.</p>","SUPPORT":"<p>Use the prefilled resilience row, Software Developer model, BLS extraction guide, bilingual headers, and read-aloud for the scenario.</p>","FALLBACK":"<p>If a live source fails, use the teacher’s dated key and name the source on the sheet. Absent students receive the Day 2 model data and complete two remaining careers.</p>"},
