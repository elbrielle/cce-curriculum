    ...
    for day in range(1, 6):
        ...
        audits.append(
            DayAudit(
                ...
                explicit_topic=bool(re.search(r"\btopic\b", teacher + day_source, re.I)),
                explicit_objective=bool(re.search(r"lesson objective|\bobjective\b", teacher + day_source, re.I)),
                explicit_dol=bool(re.search(r"demonstration of learning|\bDOL\b", teacher + day_source, re.I)),
                teks_visible=bool(re.search(r"\bTEKS?[^\n]{0,24}?d\(", teacher + day_source, re.I)),
                ...
                student_start_and_done_contract=bool(re.search(r"Today you will|What you will do|I can:", student, re.I) and re.search(r"You are done when|Done when|Show my learning:", student, re.I)),
...
def write_report(audits: list[DayAudit]) -> None:
    ...
    report = [
        ...
        "",
        "### Repaired - Grade 7 Xello selection and dependency chain across 1SW",
        "",
        "The authenticated Grade 7 configuration assigns Matchmaker and Personality Style. Live demo-student evidence shows Matchmaker collects its opening after-high-school response inline, so the separate Grade 8 After High School Goal task is not assigned. The original-scope What is CTE task and Grade 7 Matchmaker run in 1SW Wk1; Personality Style follows in Wk2, where students also save three careers once as prerequisite support for later selected Grade 7 lessons.",
        "",
        "### Repaired - Authoritative S&S and production source agree across the 1SW Xello windows",
        "",
        "Both S&S copies now carry the repaired sequence: What is CTE/Matchmaker, Personality Style plus three-career prerequisite support, Learning Style quiz/Learning styles lesson, and Add skills. Wk0 contains no Xello completion task. Add interests appears only as just-in-time prerequisite support before the later Grade 7 Interests lesson; Favorite clusters is removed.",
        "",
        ...
        "- Day 5 correctly uses FYF printed pp. 4-5 for `Perks, Neutrals, and Quirks`. FYF p. 5 is the single response home; the equivalent paper table is used only when the workbook is unavailable.",
        ...
        "",
        "**Wk0 verdict:** workbook page grounding and the daily learning contract pass. Wk0 has no Xello completion task; Matchmaker collects its opening goal response inside the Week 1 flow.",
        "",
