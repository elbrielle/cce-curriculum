...
async def main():
    ...
    async with httpx.AsyncClient(headers={"Authorization":f"Bearer {token}"},timeout=120) as c:
        ...
        td={
          1:{"TITLE":"Health Science and Nursing Routes","SUBTITLE":"50 minutes · TEKS d(1)(B), d(1)(C)","ALERT":"<strong>Keep each source label attached.</strong> FYF pp. 84-85 are the assigned district-customized HQIM snapshot. The current Singley Academy webpage is a separately dated cross-check. A difference does not make the workbook response wrong or guarantee a student outcome.","PREP":f'<ul><li>Open the two embedded workbook visuals.</li><li>Print/post the <a href="/courses/{COURSE_ID}/files/{files["ROUTE"]["id"]}/preview">route guide</a> and page 1 of the <a href="/courses/{COURSE_ID}/files/{files["COMPARE"]["id"]}/preview">comparison</a>.</li><li>Optional: preflight Xello for a current local salary check.</li></ul>',"EVIDENCE":"<p>Collect the completed nursing assistant model check, guided LVN route statement, and Stop and Jot comparison. Pay facts must retain year, geography, and measure.</p>","FLOW":flow("#5a2d91","Healthcare worker warm-up · 5 minutes","List roles beyond physician and name one task.")+flow("#4a9d2f","Health fair decision · 10 minutes","Connect one booth to the worker who staffs it.")+flow("#1f617a","See one and try one · 15 minutes","Use the completed nursing assistant model, then guide the LVN evidence marks.")+flow("#e3ad19","Read the pay label · 15 minutes","Circle May 2024, underline U.S., box median, then finish the LVN route statement.")+flow("#1f617a","Comparison close · 5 minutes","Name one preparation or responsibility change."),"MONITOR":"<p>Source key: nursing assistant $39,530; LVN $62,340; RN $93,600; nurse practitioner $129,210. All are May 2024 U.S. medians. Do not call them starting or DFW pay. The LVN evidence marks are P for program/licensure, $ for the median, and R for the responsibility.</p>","SUPPORT":"<p>Use the completed model before asking students to mark the LVN evidence. Let students rehearse the Stop and Jot with a partner, then write their own response. Score the evidence difference, not English mechanics.</p>","FALLBACK":"<p>The route guide and comparison are the full no-login route. The assigned FYF page remains valid HQIM; a current website check is a separate source note.</p>"},
          2:{"TITLE":"Compare Nursing Education and Pay","SUBTITLE":"50 minutes · TEKS d(2)(A), d(2)(B), d(5)(E)","ALERT":"<strong>Do not turn the routes into a forced ladder.</strong> Students may enter, stop, or change routes. A higher median does not erase time, cost, admission, license, or work-condition trade-offs.","PREP":f'<ul><li>Return the route guide and page 1 work.</li><li>Print/post pages 2-3 of the <a href="/courses/{COURSE_ID}/files/{files["COMPARE"]["id"]}/preview">comparison and decision</a>.</li><li>Display the Texas Board of Nursing program-approval reminder.</li></ul>',"EVIDENCE":"<p>Collect the four-career comparison, RN route check, and fictional-student recommendation using preparation, source-labeled pay, and a trade-off or fact to verify.</p>","FLOW":flow("#5a2d91","Decision-information warm-up · 5 minutes","Name the facts needed before choosing.")+flow("#4a9d2f","Compare independently · 20 minutes","Annotate RN and nurse practitioner cards, then order all four medians and explain two differences.")+flow("#1f617a","Evaluate RN routes · 10 minutes","Use Avery's priorities to choose a route to investigate first and name a fact to verify.")+flow("#e3ad19","Recommend · 10 minutes","Use preparation, pay, and one trade-off or unanswered question.")+flow("#1f617a","Ranked close · 5 minutes","Defend Rank 1 and Rank 4."),"MONITOR":"<p>Full credit requires at least three careers in the salary comparison. Accept different recommendations when the evidence matches the fictional student's needs. Correct any claim that a degree guarantees a salary. For the RN route check, program approval is required; length, cost, admission, transfer, and employer preferences still need a current program-specific source.</p>","SUPPORT":"<p>Revisit the completed nursing assistant model, use the route-card mark-up directions, and keep the printed recommendation frame visible. Students may compare three roles before adding the fourth. Score evidence, not English mechanics.</p>","FALLBACK":"<p>All required facts are in the route cards and guide. Calculators and live platforms are optional.</p>"},
        ...
        contracts={
          1:{"TOPIC":"Health Science Careers","OBJECTIVE":"Students will describe the Health Science cluster and identify nursing-related career opportunities by comparing preparation and responsibility evidence.","TEKS":"d(1)(B), d(1)(C)","DOL":"Completed nursing assistant model check and guided LVN section of the Nursing Route Comparison."},
          2:{"TOPIC":"Nursing Route Choices","OBJECTIVE":"Students will compare preparation requirements and salaries across four nursing-related careers and evaluate two RN education routes using source-labeled evidence.","TEKS":"d(2)(A), d(2)(B), d(5)(E)","DOL":"Completed Nursing Route Comparison and Decision."},
        ...
        for day in range(1,6):
            ...
            if day==4: await upsert_quiz_item(c,module_id,quiz)
        items=await paged(c,f"/courses/{COURSE_ID}/modules/{module_id}/items")
        by_url={i.get("page_url"):i for i in items}; desired=[]; used=set()
        for day in range(1,6):
            header=next(i for i in items if i.get("type")=="SubHeader" and i.get("title")==f"Day {day}")
            desired.append(header["id"]); used.add(header["id"])
            for page_kind in ("teacher","student"):
                item=by_url[pages[day][page_kind]["url"]]; desired.append(item["id"]); used.add(item["id"])
            if day==4:
                practice=next(i for i in items if i.get("type")=="Quiz" and i.get("content_id")==quiz["id"])
                desired.append(practice["id"]); used.add(practice["id"])
