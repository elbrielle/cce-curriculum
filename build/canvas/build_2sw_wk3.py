...
async def main():
  ...
  <p><a href="/courses/{COURSE_ID}/files/{files['RUBRIC']['id']}/preview">Open the 16-point scoring guide</a>.</p>
        ...
        titles={1:"STUDENT: 2SW Wk3 Day 1 - Compare Nursing Routes",2:"STUDENT: 2SW Wk3 Day 2 - Choose with Evidence",3:"STUDENT: 2SW Wk3 Day 3 - Build a Training Simulator",4:"STUDENT: 2SW Wk3 Day 4 - Write a Careful Handoff",5:"STUDENT: 2SW Wk3 Day 5 - Build a Nursing Career Evidence Map"}
        td={
          1:{
            "TITLE":"Health Science and Nursing Routes","SUBTITLE":"50 minutes · TEKS d(1)(B), d(1)(C)",
            "ALERT":"<strong>Keep each source label attached.</strong> FYF pp. 84-85 are the assigned district-customized FYF workbook snapshot. The current Singley Academy webpage is a separately dated cross-check. A difference does not make the workbook response wrong or guarantee a student outcome.",
            "PREP":f'<ul><li>Open the two embedded workbook visuals and display the <a href="/courses/{COURSE_ID}/files/{files["ROUTE"]["id"]}/preview">route guide</a>.</li><li>Open the <a href="{model_quiz_url}">two-question Canvas model check</a>.</li><li>Keep pages 1-2 of the <a href="/courses/{COURSE_ID}/files/{files["COMPARE"]["id"]}/preview">print scaffold</a> available only as an access fallback.</li><li><strong>Default printing: none.</strong></li></ul>',
            "EVIDENCE":"<p>Review the two-question Canvas model check: one correct LVN evidence classification and one nursing-assistant/LVN comparison. Pay facts must retain year, geography, and measure. Accept pages 1-2 of the print scaffold under the same criteria.</p>",
            "FLOW":(
              flow("#5a2d91","Minutes 0-5 - Welcome and healthcare workforce warm-up",'''<p>Welcome students to Week 3 and project the opening healthcare warm-up prompt.</p><ul><li>Ask, <em>“When most people think of healthcare, they immediately think of physicians. List 3 healthcare professionals who are NOT doctors, and describe ONE specific task each worker performs.”</em></li><li>Collect student thoughts. Sort their suggestions into four distinct functional categories: Direct Patient Care (nurses, CNAs), Diagnostic Testing (radiologic techs, phlebotomists), Clinical Administration (health informatics), and Research/Lab Support.</li><li>Bridge with, <em>“Today we begin our deep dive into Health Science by exploring the diverse routes within the nursing profession.”</em></li></ul>''')+
              flow("#4a9d2f","Minutes 5-15 - Health fair community decision and Irving ISD pathways",'''<p>Open FYF p. 59 (Exploring the World of Health Science) and review the community health fair scenario.</p><ul><li>Facilitate a Think-Pair-Share: Pairs choose one community clinic booth (e.g., Blood Pressure &amp; Heart Health, Diabetes Education, Child Immunization, or Elder Mobility Support).</li><li>Ask: <em>“Which Health Science worker would staff this booth, and what specific advice or data would families take home?”</em></li><li>Connect to Irving ISD pathways on FYF pp. 84-85: Singley Academy offers <strong>Nursing Science</strong>, Physical Therapy, Dental, and Emergency Medical Technician (EMT). Emphasize: CTE programs provide early career exposure and foundational skills, but state licensing boards govern practice credentials.</li></ul>''')+
              flow("#1f617a","Minutes 15-30 - Four nursing roles, scopes of practice, and model check",'''<p>Distribute or project the Nursing Career Route Guide comparing four distinct levels.</p><ul><li>Model the critical difference in educational preparation and clinical scope: (1) <strong>Certified Nursing Assistant (CNA)</strong>: high school diploma + state-approved training, provides ADL (activities of daily living) care and vital sign reporting; (2) <strong>Licensed Vocational Nurse (LVN)</strong>: ~1-year vocational diploma + NCLEX-PN, delivers focused nursing care under RN/MD supervision; (3) <strong>Registered Nurse (RN)</strong>: 2-4 year ADN/BSN degree + NCLEX-RN, performs comprehensive assessment and care plans; (4) <strong>Nurse Practitioner (NP)</strong>: graduate degree (MSN/DNP) + advanced national certification, diagnoses conditions and prescribes treatments.</li><li>Demonstrate the completed Nursing Assistant model row, then direct students to classify the LVN evidence on their record or the Canvas Model Check.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 20): Verify students mark P for program/licensure, $ for pay, and R for responsibility on the LVN row.<br>• Lap 2 (Minute 27): Check that students articulate scope differences rather than just saying 'RN does harder stuff.'</li></ul>''')+
              flow("#e3ad19","Minutes 30-45 - Interpreting nursing labor data with source labels",'''<p>Direct students to the labor market summary in the Route Guide.</p><ul><li>Model annotating pay statistics: circle May 2024, underline national U.S., and box median: Nursing Assistant ($39,530), LVN ($62,340), RN ($93,600), NP ($129,210).</li><li>Address the common misconception: explain that BLS medians represent experienced workers nationwide, not entry-level salaries in North Texas hospitals.</li><li>Students complete Question 2 of the Canvas check: write a comparative sentence using the frame: <em>“The LVN route requires _____, while the nursing assistant route _____.”</em></li></ul>''')+
              flow("#24323d","Minutes 45-50 - Shortest approved route DOL and collection",'''<p>Direct students to the Day 1 Comparison Matrix Exit Ticket.</p><ul><li>Students complete the 3-row matrix for Nursing Assistant vs. LVN (Preparation, Responsibility, Median Pay).</li><li>Address the evaluative prompt: <em>“Which route better fits a student who wants the shortest approved pathway into direct patient support, and what is the trade-off in earnings and responsibilities?”</em></li><li><strong>Safe Trim:</strong> If discussion runs long, omit optional Xello salary browsing; protect the 4-role scope comparison and the 2-question model check.</li></ul>''')
            ),
            "MONITOR":"<p>Source key: nursing assistant $39,530; LVN $62,340; RN $93,600; nurse practitioner $129,210. All are May 2024 U.S. medians. Do not call them starting or DFW pay. The LVN evidence marks are P for program/licensure, $ for the median, and R for the responsibility.</p>",
            "SUPPORT":"<p>Use the completed model before the Canvas check. Let students rehearse Question 2 with a partner, then submit their own response. Keep preparation/preparación and responsibility/responsabilidad visible. Score the evidence difference, not English mechanics.</p>",
            "FALLBACK":"<p>If Canvas is unavailable, use pages 1-2 of the print scaffold. The assigned FYF workbook page remains valid; a current website check is a separate source note.</p>"},
          2:{
            "TITLE":"Compare Nursing Education and Pay","SUBTITLE":"50 minutes · TEKS d(2)(A), d(2)(B), d(5)(E)",
            "ALERT":"<strong>Canvas is the default response surface.</strong> Do not print the four-page comparison as a class set. Use it only as an optional print/access fallback. Do not turn the routes into a forced ladder.",
            "PREP":f'<ul><li>Open the route guide beside the <a href="{route_quiz_url}">Nursing Route Evidence Check</a>.</li><li>Keep the <a href="/courses/{COURSE_ID}/files/{files["COMPARE"]["id"]}/preview">four-page scaffold</a> available only for students who need or prefer print.</li><li>Display the Texas Board of Nursing program-approval reminder.</li><li><strong>Default printing: none.</strong></li></ul>',
            "EVIDENCE":"<p>Review the Canvas response for the four-career order, two evidence differences, RN route check, and fictional-student recommendation. Accept the optional print version under the same success criteria.</p>",
            "FLOW":(
              flow("#5a2d91","Minutes 0-5 - Welcome and education trade-off warm-up",'''<p>Welcome students and project the education investment prompt.</p><ul><li>Ask, <em>“A shorter training program gets you working and earning in months. A longer college degree takes years but can lead to higher autonomy and pay. What critical facts would you need to gather before deciding which path to take?”</em></li><li>Collect 3-4 student ideas: program cost, tuition debt, admission competition, clinical hour requirements, and daily work stress.</li><li>Bridge with, <em>“Today we evaluate education routes and salaries across nursing careers to make an evidence-based recommendation.”</em></li></ul>''')+
              flow("#4a9d2f","Minutes 5-25 - Comparative data analysis across four nursing roles",'''<p>Students open the Canvas Nursing Route Evidence Check and the Nursing Career Route Guide.</p><ul><li>Students rank all four nursing roles by May 2024 U.S. median pay ($39,530 CNA &rarr; $62,340 LVN &rarr; $93,600 RN &rarr; $129,210 NP).</li><li>Students write two precise comparative statements analyzing how preparation differences explain the salary gaps.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 10): Confirm all 4 roles are correctly ordered with exact May 2024 dollar figures.<br>• Lap 2 (Minute 20): Check explanations—ensure students link specialized clinical training and legal licensing (NCLEX) to increased earnings.</li></ul>''')+
              flow("#1f617a","Minutes 25-35 - Evaluating RN education routes and Texas Board of Nursing approval",'''<p>Analyze the two primary educational routes to becoming a Registered Nurse: <strong>Associate Degree in Nursing (ADN)</strong> vs. <strong>Bachelor of Science in Nursing (BSN)</strong>.</p><ul><li>Review key differences: ADN (2 years at community college, lower cost, fast hospital entry) vs. BSN (4 years at university, broader leadership/public health training, preferred by magnet hospitals). Both take the exact same licensing exam: NCLEX-RN.</li><li>Emphasize Texas regulatory requirements: Prospective students must verify that a nursing program is officially approved by the <strong>Texas Board of Nursing (BON)</strong> before enrolling; unapproved programs prevent graduates from testing for licensure.</li></ul>''')+
              flow("#e3ad19","Minutes 35-45 - Defensible fictional student recommendation",'''<p>Students select one fictional student profile (Jordan, Avery, or Sam) with unique life goals and financial constraints.</p><ul><li>Students author a structured recommendation defending their chosen route using three required elements: (1) One preparation requirement, (2) One correctly labeled pay statistic, and (3) One realistic trade-off or unanswered question.</li></ul>''')+
              flow("#24323d","Minutes 45-50 - Ranked justification DOL and close",'''<p>Direct students to the Day 2 Exit Ticket.</p><ul><li>Students rank all four careers from best to least suitable for their chosen fictional student, providing concrete evidence for Rank 1 and Rank 4.</li><li>Students identify the single trade-off that carried the greatest weight in their decision.</li><li><strong>Safe Trim:</strong> At minute 35, transition directly to the fictional recommendation. Accept bulleted pros/cons in place of extended narrative if pacing is tight.</li></ul>''')
            ),
            "MONITOR":"<p>Full credit requires at least three careers in the salary comparison. Accept different recommendations when the evidence matches the fictional student's needs. Correct any claim that a degree guarantees a salary. For the RN route check, program approval is required; length, cost, admission, transfer, and employer preferences still need a current program-specific source.</p>",
            "SUPPORT":"<p>Keep the point-of-use word banks and complete frames visible in the Canvas check. Students may rehearse Questions 2, 3, and 5 orally before typing. Offer the four-page scaffold only when print or enlarged handwriting space improves access. Score evidence, not English mechanics.</p>",
            "FALLBACK":"<p>If Canvas is unavailable, use the four-page scaffold. All required facts are in the route cards and guide; no Xello or H&amp;L login is required for this DOL.</p>"},
          3:{
            "TITLE":"Build a Vital-Signs Training Simulator","SUBTITLE":"50 minutes · TEKS d(1)(C)",
            "ALERT":"<strong>Simulation only.</strong> No student measurements, diagnoses, or medical-device claims. The physical micro:bit, browser simulator, and paper trace are equal routes.",
            "PREP":f'<ul><li>Test MakeCode on the student network and open the completed Button A model already embedded in the Student Guide.</li><li><strong>Default grouping:</strong> pairs, one connected device per pair. Use one optional micro:bit per pair only when hardware is ready; the browser simulator is the default.</li><li>Post the <a href="/courses/{COURSE_ID}/files/{files["BUILD"]["id"]}/preview">two-page build and test guide</a>. Provide one copy per student only when paper is the response route.</li><li>Assign Driver and Code Reader. Switch roles after Button A passes.</li></ul>',
            "EVIDENCE":"<p>Collect one test record per student plus one screenshot/share link per pair, or one completed paper block trace per student. Hardware access is not graded.</p>",
            "FLOW":(
              flow("#5a2d91","Minutes 0-5 - Welcome and measurement vs. simulation warm-up",'''<p>Welcome students, seat them with coding partners, and project the tech boundary prompt.</p><ul><li>Ask, <em>“What is the fundamental difference between a medical device that measures a live human body and a computer program that displays a simulated number?”</em></li><li>Collect student answers. Establish the non-negotiable boundary: The micro:bit device displays randomly generated training numbers; it does NOT measure heart rate, diagnose illness, or replace a medical professional.</li><li>Bridge with, <em>“Healthcare workers train on high-tech clinical simulators before treating real patients. Today you build and test a vital-signs simulator using MakeCode.”</em></li></ul>''')+
              flow("#4a9d2f","Minutes 5-13 - Vital-sign diagnostic tools and clinical data",'''<p>Open FYF p. 60 (Vitals in Motion) and Climber Notes slide 2.</p><ul><li>Review the four fundamental diagnostic tools: (1) Blood pressure cuff/sphygmomanometer, (2) Pulse oximeter (oxygen saturation and pulse), (3) Digital thermometer, and (4) Stopwatch (respiratory rate).</li><li>Match each instrument to its clinical measurement unit (bpm, % SpO2, °F, mmHg). Reiterate: Zero student health data will be measured or collected in class.</li></ul>''')+
              flow("#1f617a","Minutes 13-21 - Model Button A in MakeCode",'''<p>Demonstrate the complete Button A coding structure in MakeCode using the embedded Student Guide visual.</p><ul><li>Step 1: Event block: <code>on button A pressed</code>.</li><li>Step 2: Variable assignment: set <code>heartRate</code> to <code>pick random 60 to 110</code>.</li><li>Step 3: Output: <code>show number heartRate</code>.</li><li>Step 4: Animation: display heart icon &rarr; <code>pause 200 ms</code> &rarr; clear screen/small heart.</li><li>Test once on the browser simulator to verify code execution.</li></ul>''')+
              flow("#e3ad19","Minutes 21-42 - Paired build, role switch, and Button B testing",'''<p>Pairs execute the build using Driver and Code Reader roles.</p><ul><li><strong>Phase 1 (10 min):</strong> Driver builds Button A while Code Reader tracks the logic. Test and record results.</li><li><strong>Switch Roles (10 min):</strong> Partner B becomes Driver to build Button B: temperature simulation. Code a random integer between <code>970</code> and <code>1005</code> (representing 97.0°F to 100.5°F, where 986 = 98.6°F) and display the value.</li><li>Optional extension for rapid finishers: Add a conditional alert (if heart rate &gt; 100, show 'R' for report).</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 26): Confirm Button A generates varied integer values between 60 and 110.<br>• Lap 2 (Minute 34): Check Button B range (970 to 1005) and verify students understand why 3-digit whole numbers are used on the 5x5 LED grid.<br>• Lap 3 (Minute 39): Ask pairs: <em>“Where did that heart rate number come from?”</em> Ensure students reply <em>“the code algorithm,”</em> not a human body.</li></ul>''')+
              flow("#24323d","Minutes 42-50 - Boundary check DOL, cleanup, and submission",'''<p>Direct students to the Day 3 Exit Ticket.</p><ul><li>Scenario: <em>“The simulator displays 108 followed by 'R'. Does this prove a patient has tachycardia? What should the student record, and what is the proper supervised next step if a fictional patient reports chest pain?”</em></li><li>Students save MakeCode share links or take project screenshots; paper-trace students submit completed block guides.</li><li><strong>Safe Trim:</strong> Omit the optional conditional alert (&gt;100 check); fiercely protect Button A and B builds, the hardware/simulation boundary, and the nursing connection.</li></ul>''')
            ),
            "MONITOR":"<p><strong>Lap 1—Button A:</strong> target = event, fictional-value variable, and display blocks in sequence. Feedback: “Point to the block that creates the value.” If three or more pairs cannot identify it, pause and reread the embedded model top to bottom. <strong>Lap 2—Button B:</strong> target = 970-1005 range and a displayed temperature code. If several students type 97-100.5, model why this micro:bit build uses whole-number codes. <strong>Lap 3—Explanation:</strong> target = code as the source, not a person. If students claim measurement or diagnosis, return to the boundary frame before collection.</p><p><strong>Safe trim:</strong> cut the optional &gt;100 alert and pulse animation refinement first. Protect both button tests, the simulator boundary, and the nursing-role connection.</p>",
            "SUPPORT":"<p>The Student Guide supplies the completed Button A visual, point-of-use words, and a full test/revision frame. Partners may rehearse the boundary explanation, but each student records a complete thought. Do not make the Code Reader a passive observer; that student points to the next block and checks the test record.</p>",
            "FALLBACK":"<p>MakeCode failure: use the completed paper block trace in the same two-page guide and test the written sequence against the same four checks. Hardware failure: remain in the browser simulator. Recovery: rename the project, take a screenshot before closing, and copy the share link into the assigned submission or class collection location. Neither route changes the score.</p>"},
          4:{
            "TITLE":"Document a Fictional Patient Handoff","SUBTITLE":"50 minutes · TEKS d(1)(C)",
            "ALERT":"<strong>Fictional information only.</strong> Do not collect names, symptoms, temperatures, oxygen levels, blood pressure, or health histories from students. Record observations, not diagnoses.",
            "PREP":f'<ul><li><strong>Default grouping:</strong> pairs for card analysis; individual writing for the handoff.</li><li>Print one four-card <a href="/courses/{COURSE_ID}/files/{files["CARDS"]["id"]}/preview">fictional-card set</a> per eight students (four pairs), cut into four cards. Print one two-page <a href="/courses/{COURSE_ID}/files/{files["HANDOFF"]["id"]}/preview">handoff record</a> per student.</li><li>Project the <a href="/courses/{COURSE_ID}/files/{files["RUBRIC"]["id"]}/preview">rubric</a>; print individual rubric copies only when students will mark them.</li><li>Open the unpublished <a href="{quiz_url}">practice quiz</a>. Use it after the handoff, not as a replacement.</li></ul>',
            "EVIDENCE":"<p>Collect one individual handoff per student. One pair may share a card, but not a response. This is the recommended 16-point minor checkpoint when the gradebook groups are ready.</p>",
            "FLOW":(
              flow("#5a2d91","Minutes 0-5 - Welcome and observation vs. diagnosis warm-up",'''<p>Welcome students and project the clinical documentation prompt.</p><ul><li>Ask, <em>“Read these two statements: (A) 'The patient's heart rate was 132 beats per minute and breathing was rapid.' (B) 'The patient has serious heart disease.' Which statement belongs in a nursing observation log, and why?”</em></li><li>Collect responses. Teach the critical standard: Statement A records objective, observable data. Statement B is an unsupported clinical diagnosis outside a nursing assistant's or student's scope of practice.</li><li>Bridge with, <em>“Today you analyze fictional clinical cases and write an accurate, objective patient handoff report.”</em></li></ul>''')+
              flow("#4a9d2f","Minutes 5-13 - The Five-Part Clinical Handoff routine",'''<p>Project the standard 5-part SBAR-aligned handoff model (FYF p. 61).</p><ul><li>1. <strong>Situation:</strong> Location, fictional patient identifier, and immediate concern.</li><li>2. <strong>Observations:</strong> Objective measurements (temperature, pulse) and exact patient-quoted statements.</li><li>3. <strong>Comparison:</strong> How do current findings compare to baseline or previous rest values?</li><li>4. <strong>Handoff:</strong> Name the specific licensed supervisor receiving the report (e.g. Charge Nurse, Attending RN).</li><li>5. <strong>Reasoning:</strong> Concrete case facts that justify immediate follow-up.</li><li>Review the 16-point Major Rubric criteria.</li></ul>''')+
              flow("#1f617a","Minutes 13-30 - Four fictional case patterns analysis",'''<p>Distribute the four Fictional Patient Handoff Cards to student pairs.</p><ul><li>Analyze case patterns: (1) <strong>Card A</strong>: Elevated vitals following physical exertion that normalize after rest; (2) <strong>Card B</strong>: Subtle changes paired with urgent reported symptoms requiring prompt escalation; (3) <strong>Card C</strong>: 'Normal' numeric vitals that contradict severe dizziness and nausea; (4) <strong>Card D</strong>: Conflicting readings that indicate equipment malfunction or improper cuff placement.</li><li>Students annotate their card: underline exact numbers in blue, circle patient quotes in green, and cross out any speculative medical guesses.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 18): Confirm students separate measured numbers from patient quotes.<br>• Lap 2 (Minute 26): Intervene if students write medical diagnoses on Card B or C; redirect to observable physical signs.</li></ul>''')+
              flow("#e3ad19","Minutes 30-43 - Author individual Clinical Handoff Record",'''<p>Students work independently to write their formal Patient Handoff Record.</p><ul><li>Draft all five sections using precise, non-diagnostic clinical language.</li><li>Include at least two specific case facts from their assigned card.</li><li>Self-audit using the 16-point rubric and revise at least one sentence for greater objectivity or clarity.</li></ul>''')+
              flow("#24323d","Minutes 43-50 - Clinical data-quality DOL and submission",'''<p>Direct students to the Day 4 Exit Ticket.</p><ul><li>Scenario: <em>“A patient's blood pressure reading swings from 118/76 to 160/110 to 90/60 within 30 seconds, but the patient remains calm and reports feeling completely fine. What data-quality problem must the worker suspect, what should be rechecked, and how should this be reported to the RN?”</em></li><li>Collect all completed Clinical Handoff Records for Major scoring.</li><li><strong>Safe Trim:</strong> Move the optional practice quiz to an asynchronous review; protect the individual handoff record, rubric revision, and exit scenario.</li></ul>''')
            ),
            "MONITOR":"<p><strong>Lap 1—evidence sort:</strong> target = values and exact reported words are marked separately from inference. Feedback: “Which words came directly from the card?” If several pairs diagnose, pause and revise one statement together. <strong>Lap 2—handoff:</strong> target = two case facts, a supervised receiver, and no diagnosis. If several students omit the person’s report, display the complete frame and require that sentence before they continue. <strong>Lap 3—reasoning:</strong> target = action supported by two facts, not urgency guessed from one number.</p><p>Card A = recovery after rest. Card B = prompt supervised handoff without diagnosis. Card C = retain the reported dizziness. Card D = recheck the device/process and report the conflict. <strong>Safe trim:</strong> cut or move the practice quiz to the next opening; protect the individual handoff and one evidence-based revision.</p>",
            "SUPPORT":"<p>The complete handoff frame and bilingual terms now sit beside the five-part routine in the Student Guide. Color-code values, quoted symptoms, and inference. Students may rehearse with a partner or use speech-to-text, but submit a private individual response. Score evidence, not English mechanics.</p>",
            "FALLBACK":"<p>No equipment is needed. Absent students use one fictional card and the same individual record. If printing is unavailable, students type the five labeled parts in Canvas. Real health concerns follow the school nurse or emergency process, not the simulation.</p>"},
          5:{
            "TITLE":"Build a Nursing Career Evidence Map","SUBTITLE":"50 minutes · TEKS d(1)(C)",
            "ALERT":"<strong>Use one evidence basis.</strong> Students compare the fixed nursing careers with the year, geography, and measure attached to every pay figure. No saved-career prerequisite, profile screenshot, or platform completion is required.",
            "PREP":f'<ul><li>Open the fixed Nursing Career Route Guide and one completed comparison model.</li><li><strong>Default grouping:</strong> individual evidence map with one brief partner rehearsal.</li><li>Provide one <a href="/courses/{COURSE_ID}/files/{files["REFLECTION"]["id"]}/preview">Nursing Career Evidence Map</a> per student as paper or digital annotation. Do not print a second packet.</li></ul>',
            "EVIDENCE":"<p>Collect one individual map comparing at least three nursing careers, one supported current-fit statement, and one exact fact to verify next. Do not grade H&amp;L or eDynamic clicks.</p>",
            "FLOW":(
              flow("#5a2d91","Minutes 0-5 - Welcome and healthcare exploration warm-up",'''<p>Welcome students, seat them with their weekly artifacts, and project the reflection launch prompt.</p><ul><li>Ask, <em>“Name ONE healthcare career you are genuinely curious to keep investigating, and ONE specific factual question you would need answered before committing to that career path.”</em></li><li>Hear 2-3 quick shares. Validate non-nursing medical interests (radiology, dental, surgical tech) while grounding today's evidence work in our core Health Science benchmarks.</li><li>Bridge with, <em>“Today we synthesize our week's learning, cross-check labor evidence, and construct our Nursing Career Evidence Map.”</em></li></ul>''')+
              flow("#4a9d2f","Minutes 5-25 - Comprehensive audit of three nursing careers",'''<p>Students reopen their Nursing Career Route Guide and prior comparison notes.</p><ul><li>Students select THREE distinct nursing roles and audit four core evidence dimensions: (1) Daily Clinical Responsibilities, (2) Mandatory Educational &amp; Licensure Preparation, (3) Authentic Work Environments &amp; Physical Demands, and (4) Source-Labeled Pay (May 2024 BLS U.S. median).</li><li>Enforce source integrity: Students must verify that every salary entry preserves its survey year, geographic scope, and median metric.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 12): Confirm students have selected 3 careers and logged accurate daily duties.<br>• Lap 2 (Minute 20): Check preparation fields—verify students differentiate between postsecondary certificates, associate degrees, and university bachelors.</li></ul>''')+
              flow("#1f617a","Minutes 25-42 - Synthesizing personal career alignment and evidence map",'''<p>Students complete their Nursing Career Evidence Map.</p><ul><li>Identify: (1) One career that currently aligns with personal strengths, (2) One concrete piece of evidence (e.g. interest in patient connection, desire for technical problem solving) that supports the fit, (3) One career ruled out or needing more investigation, and (4) The exact piece of information to research next.</li><li>Optional enrichment: Students who complete their core map may cross-check local DFW hospital career postings or explore the Irving ISD Singley Academy healthcare course sequence.</li></ul>''')+
              flow("#24323d","Minutes 42-50 - Nursing career concept map DOL and close",'''<p>Direct students to the Day 5 Exit Ticket Concept Map.</p><ul><li>Students construct a 4-node diagram around their central selected career: Connect to (1) One Daily Responsibility, (2) One Preparation/Licensure Step, (3) One Work Condition or Pay Fact, and (4) One Reason to Pursue or Rule Out.</li><li>Star the single connection that represents the strongest verified evidence.</li><li>Collect completed Evidence Maps and exit tickets.</li><li><strong>Safe Trim:</strong> Provide the Nursing Assistant row pre-filled as a reference model so students only audit two additional roles, preserving individual alignment claims and the concept map exit ticket.</li></ul>''')
            ),
            "MONITOR":"<p><strong>Minute 10:</strong> target = three career names and one responsibility for each. <strong>Minute 25:</strong> target = preparation, work condition, and source-labeled pay entered for all three. <strong>Minute 40:</strong> target = current-fit claim plus one exact next fact. <strong>Safe trim:</strong> provide the completed nursing-assistant row and require two additional rows; protect the supported current-fit statement and question to verify.</p>",
            "SUPPORT":"<p>Highlight the same four evidence fields on each career row. Keep compare/comparar, responsibility/responsabilidad, preparation/preparación, and work condition/condición de trabajo visible. Allow oral rehearsal before the final statement.</p>",
            "FALLBACK":"<p>Use the fixed guide and the paper or Canvas response home your teacher assigned. An absent student completes the same comparison.</p>"}
        }
        contracts={
          ...
          5:{"TOPIC":"Nursing Career Evidence","OBJECTIVE":"Students will compare at least three nursing-related careers and explain one current fit using responsibility, preparation, work-condition, and source-labeled pay evidence.","TEKS":"d(1)(C)","DOL":"Nursing Career Evidence Map with one supported current-fit statement and one next fact to verify."}}
        pages={}
        for day in range(1,6):
