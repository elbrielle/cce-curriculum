...
async def main():
    ...
    async with httpx.AsyncClient(headers={"Authorization":f"Bearer {token}"},timeout=120) as c:
        ...
        student_values={
          ...
          2:{"COMPARE_FILE_ID":files["COMPARE"]["id"],"ROUTE_QUIZ_URL":route_quiz_url},
        ...
        for day in range(1,6):
            ...
            if day==1:
                model_check=next(i for i in items if i.get("type")=="Quiz" and i.get("content_id")==model_quiz["id"])
                desired.append(model_check["id"]); used.add(model_check["id"])
            if day==2:
                route_check=next(i for i in items if i.get("type")=="Quiz" and i.get("content_id")==route_quiz["id"])
                desired.append(route_check["id"]); used.add(route_check["id"])
            if day==4:
