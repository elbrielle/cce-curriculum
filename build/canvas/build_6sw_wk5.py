...
async def main():
    ...
    async with httpx.AsyncClient(headers={"Authorization": f"Bearer {token}"}, timeout=120) as client:
        ...
        for day in range(1, 5):
            ...
            # Canvas removes online submission routes when Display Grade is
            # Not Graded. A 0-point Percentage item that is explicitly omitted
            # from the final grade preserves annotation/upload/text evidence
            # without affecting the district grade.
        ...
        required_major_routes = {"student_annotation", "online_upload", "online_text_entry", "media_recording"}
        if not required_major_routes.issubset(set(major.get("submission_types") or [])) or major.get("annotatable_attachment_id") is None:
            raise RuntimeError("Job Skills Major private-route mismatch")
