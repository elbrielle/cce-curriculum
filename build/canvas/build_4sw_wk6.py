...
STUDENT_WORKSHEET_COPY_URLS = {
    (1, 'the three-page no-workbook fallback'): "https://docs.google.com/document/d/1RaEv6-7sMIEAUdkF-uR3PrQYYac1AD_GV-YZ5bymg0g/copy",
    (2, 'the four-page paper or enlarged fallback'): "https://docs.google.com/document/d/162w0aB4opkQzWqjp3F1GY20H3o2jnKKCAzey3MGSwYs/copy",
    (3, 'the three-page paper fallback'): "https://docs.google.com/document/d/1rXBZT9JFJKlLkUgefOJJax9eYdWEo1i8LiCphp_LxLM/copy",
    (4, 'the three-page paper fallback'): "https://docs.google.com/document/d/1DnzXEuQjtxPAWJjz7RvJFMGzlD4zJ7Sy9sjRh8s-Ymc/copy",
    (5, 'the four-page paper fallback'): "https://docs.google.com/document/d/1yMiMOaAYxi4DE0WDzX0i-OoDlqOeVwZhAfkTO6ZVoX0/copy",
}
    ...
    async with httpx.AsyncClient(headers={"Authorization": f"Bearer {token}"}, timeout=120) as client:
        ...
        teacher = {
            1: {
                ...
                "FLOW": (
                    flow("#5a2d91", "Minutes 0-5 - Welcome and clue vs. conclusion warm-up", '''<p>Welcome students to Week 6 of 4SW and project the analytical reasoning prompt.</p><ul><li>Ask students: <em>“If a commercial delivery truck's engine temperature light flashes red on the highway, does that clue prove that the water pump is broken, or does it only prove that coolant temperature exceeded a safety threshold? What is the danger of replacing parts based on a guess rather than diagnostic testing?”</em></li><li>Collect 2-3 student thoughts: A warning light indicates a symptom (high temperature), not the root cause (could be a broken belt, coolant leak, stuck thermostat, or faulty sensor). Replacing parts blindly wastes money and leaves vehicles stranded.</li><li>Bridge with, <em>“Analytical reasoning requires separating observed facts from unverified conclusions. Today we examine FYF pp. 153-155 and evaluate four truck symptom clue sets to establish safe maintenance priorities.”</em></li></ul>''')
                    + flow("#4a9d2f", "Minutes 5-12 - Diagnostic tool limits &amp; modeling clue vs. limit vs. safe action", '''<p>Direct students to the diagnostic tools overview on FYF p. 153 and model objective documentation:</p><ul><li>Analyze Diagnostic Tool Boundaries:<br>1. <strong>Dashboard Warning Lights:</strong> Alert the operator to an out-of-spec condition; they do NOT identify the exact damaged component.<br>2. <strong>OBD-II Diagnostic Trouble Code (DTC) Readers:</strong> Read electronic sensor fault codes (e.g., P0128); they tell you which electrical circuit reported an error, not whether the sensor or mechanical part failed.<br>3. <strong>Direct Physical Inspection:</strong> Laser thermometers, pressure testers, and multimeter checks are required to confirm root causes.</li><li>Model Exemplar 3-Part Response Frame:<br>• <strong>Clue:</strong> <em>“Engine temperature gauge reads 245°F and steam is visible at the reservoir.”</em><br>• <strong>Limit:</strong> <em>“This symptom does not prove the radiator is cracked; the cause could be a broken serpentine belt or leaking hose.”</em><br>• <strong>Safe Action:</strong> <em>“Immediately park the vehicle in a safe location, turn off the engine, notify fleet dispatch, and wait for cooling system pressure release before inspection.”</em></li><li>Review Non-Example Overclaim: <em>“The head gasket is blown and the truck needs a whole new engine.”</em></li></ul>''')
                    + flow("#1f617a", "Minutes 12-30 - Four symptom clue sets &amp; priority ranking sprint", '''<p>Students analyze the four vehicle clue sets on FYF pp. 154-155 (or 3-page companion):</p><ul><li>Issue 1 (Engine Lubrication / Oil Pressure): Oil pressure light on + low dipstick reading.</li><li>Issue 2 (Electrical / Charging System): Battery warning light + dimming headlights at idle.</li><li>Issue 3 (Cooling / Overheating): Temperature gauge spiked + coolant odor.</li><li>Issue 4 (Tire / Steering / Suspension): Pulling hard to the right + uneven tread wear.</li><li>Rank Issues by Immediate Safety Hazard: Assign Priority 1 (Immediate Stop Required) to Priority 4 (Scheduled Maintenance).</li><li>Draft Two Operational Action Plans: Detail immediate driver action, dispatch notification, and technician diagnostic check.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 18): Ensure students describe observable facts rather than jumping to part replacements.<br>• Lap 2 (Minute 26): Verify Priority 1 is assigned to catastrophic safety hazards (e.g., loss of oil pressure or brake failure).</li></ul>''')
                    + flow("#e3ad19", "Minutes 30-43 - Cross-career skill transfer: Medical &amp; IT parallels", '''<p>Guide students to examine how symptom-versus-root-cause reasoning transfers across careers:</p><ul><li>Prompt: <em>“How does an ER doctor or a cybersecurity network analyst use this exact same three-part reasoning (Symptom &rarr; Boundary &rarr; Safe Immediate Action) when responding to a fever or a server alert?”</em></li><li>Students record one cross-career parallel on their reflection sheet.</li></ul>''')
                    + flow("#1f617a", "Minutes 43-50 - Exit check and Week 6 evidence folder storage", '''<p>Students submit their private Canvas exit response and store their workbook/packet.</p><ul><li><strong>Safe Trim:</strong> Shorten cross-career discussion; fiercely protect the 4 clue analyses, priority ranking, two action plans, and private exit response.</li></ul>''')
                ),
            ...
            2: {
                ...
                "FLOW": (
                    flow("#5a2d91", "Minutes 0-5 - Welcome and the transferable skill claim warm-up", '''<p>Welcome students, seat them with devices or the 4-page reference packet, and project the transferable skills prompt.</p><ul><li>Ask students: <em>“If someone writes on a job resume, 'I have amazing communication and problem-solving skills,' why is an employer unimpressed unless that applicant describes the EXACT tasks they performed?”</em></li><li>Collect 2-3 student thoughts: Anyone can list buzzwords; proof requires concrete behavioral evidence of what you actually did, what tools you used, and what result you achieved.</li><li>Bridge with, <em>“Transferable skills are proven through task evidence, not empty claims. Today we compare four foundational career skills across six distinct technical occupations.”</em></li></ul>''')
                    + flow("#4a9d2f", "Minutes 5-13 - Deconstructing four core transferable skills &amp; task modeling", '''<p>Display the Four Core Skill Domains on the main screen:</p><ul><li>1. <strong>Analytical Reasoning:</strong> Breaking down complex data, isolating variables, and identifying failure causes before taking action.</li><li>2. <strong>Precise Technical Communication:</strong> Conveying critical specifications, measurements, and status reports without ambiguity.</li><li>3. <strong>Systems Thinking:</strong> Understanding how interconnected components, workflows, and feedback loops impact total performance.</li><li>4. <strong>Active Listening &amp; Observation:</strong> Paying rigorous attention to subtle sensory, verbal, or operational cues to anticipate errors.</li><li>Model Claim vs. Proof Comparison:<br>• <strong>Weak Claim:</strong> <em>“A software developer and an auto technician both solve problems.”</em><br>• <strong>Evidentiary Proof:</strong> <em>“A software developer uses analytical reasoning to read error trace logs and isolate broken code syntax before deploying an update. Similarly, an automotive technician uses analytical reasoning to read OBD-II sensor waveforms and isolate an intermittent misfire before replacing an ignition coil. Both isolate root causes using diagnostic telemetry rather than guessing.”</em></li></ul>''')
                    + flow("#1f617a", "Minutes 13-38 - Six-career comparative task evidence sprint", '''<p>Students analyze four skills across six career cards (Aviation Pilot, Drone Specialist, Auto Tech, Software Dev, Nurse, Construction Mgr):</p><ul><li>Select 2 Distinct Occupations per Skill Domain: Document the specific daily task where the worker applies that skill.</li><li>Identify Required Equipment / Data Sources: State what software, tools, or meters the worker uses to execute that task.</li><li>Synthesize a 3-Task Pattern Claim: Explain how one skill functions consistently across three completely different career clusters.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 22): Ensure students describe specific observable tasks, not generic job descriptions.<br>• Lap 2 (Minute 32): Confirm students name concrete tools, software, or data readouts for each career.</li></ul>''')
                    + flow("#e3ad19", "Minutes 38-45 - Personal skill inventory &amp; 8th-grade academic transfer", '''<p>Students reflect on their own 8th-grade coursework:</p><ul><li>Identify ONE transferable skill they demonstrated during 4SW (e.g., during airport design, drone testing, or course planning) and explain how it will help them succeed in high school.</li></ul>''')
                    + flow("#1f617a", "Minutes 45-50 - Submit Transferable Skills Evidence in Canvas", '''<p>Students submit their completed evidence in Canvas or turn in their 4-page packet.</p><ul><li><strong>Safe Trim:</strong> Shorten personal reflection; fiercely protect all four skill-to-career comparisons, the 3-task pattern claim, and private submission.</li></ul>''')
                ),
            ...
            3: {
                ...
                "FLOW": (
                    flow("#5a2d91", "Minutes 0-5 - Welcome and professional organization warm-up", '''<p>Welcome students, seat them with devices or the 3-page reference guide, and project the organizational types prompt.</p><ul><li>Ask students: <em>“What is the difference between a government agency that enforces federal laws, an independent credentialing board that gives certification tests, and a student club that competes at the state leadership conference?”</em></li><li>Collect 2-3 student thoughts: Legal authority vs. skill testing vs. student leadership development.</li><li>Bridge with, <em>“Navigating your career requires knowing who makes the rules, who issues credentials, and who provides professional networking. Today we categorize six key organizations across four distinct types.”</em></li></ul>''')
                    + flow("#4a9d2f", "Minutes 5-15 - The Four Organization Types &amp; access boundary deconstruction", '''<p>Guide students through the Four Organization Types on the main display:</p><ul><li>1. <strong>Government Regulatory Agency (e.g., FAA):</strong> Federal/state government body established by law. Writes legal regulations, enforces public safety, controls national airspace/licensing. You do not 'join as a member'; you must comply with their rules.</li><li>2. <strong>Industry Credentialing Organization (e.g., ASE):</strong> Independent nonprofit standards organization. Writes technical competency exams and grants industry-recognized certifications to qualified individuals.</li><li>3. <strong>Career &amp; Technical Student Organization - CTSO (e.g., SkillsUSA, TSA):</strong> School-based co-curricular leadership organizations recognized by TEA. Students participate through local high school CTE chapters, develop workplace skills, and compete in state/national contests.</li><li>4. <strong>Professional Association (e.g., NSPE, AOPA):</strong> Voluntary membership associations for working professionals and aspiring students. Provides industry journals, networking conferences, mentorship, and continuing education.</li><li>Privacy &amp; Safety Rule: Students never sign up for real accounts or share personal contact data during class; exploration is informational.</li></ul>''')
                    + flow("#1f617a", "Minutes 15-33 - Six-organization classification sprint &amp; Sam case study", '''<p>Students audit the six organization cards in their guide:</p><ul><li>Classify FAA, ASE, SkillsUSA, TSA, NSPE, and AOPA into their correct organizational category.</li><li>Analyze the Fictional Sam Case Study: Sam is an 8th-grader interested in engineering and aviation who wants hands-on competitions now and professional industry connections in the future. Which organization should Sam explore in high school (e.g., TSA or SkillsUSA), and which professional association should Sam bookmark for college?</li><li>Formulate ONE question about high school CTSO chapter membership in Irving ISD.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 22): Check that students do not confuse the FAA (regulatory government) with a student club or membership group.<br>• Lap 2 (Minute 29): Verify students identify an Irving ISD high school CTSO route for Sam.</li></ul>''')
                    + flow("#e3ad19", "Minutes 33-43 - Independent practice: Organization Type Check Quiz", '''<p>Students complete the 5-question retryable Canvas Practice Quiz: 'Career Organization Type Check.'</p><ul><li>Assess understanding of regulatory vs. credentialing vs. CTSO functions.</li></ul>''')
                    + flow("#1f617a", "Minutes 43-50 - Exit check and Week 6 folder storage", '''<p>Students record their Sam recommendation and CTSO question on their exit card.</p><ul><li><strong>Safe Trim:</strong> Trim whole-class case review; fiercely protect the 6-organization classification, Sam defense, and Canvas Quiz completion.</li></ul>''')
                ),
            ...
            4: {
                ...
                "PREP": f'<ul><li>Provide one device per student; no grouping or employment history is required. The four cases, six-item Quiz, and class-artifact model are supplied.</li><li>Print one {link(files["INTEGRITY"]["id"], "three-page paper fallback")} only for each no-device or enlarged-route student.</li><li>Project the supplied Day 2 artifact model. Students submit the Quiz, including Q6, or one packet; Q6 is the exit.</li><li>Ask students to open Entry 4 of the CCE Six-Weeks Evidence Log from their CCE binder or teacher-designated digital folder. Do not collect it.</li></ul>',
                ...
                "FLOW": (
                    flow("#5a2d91", "Minutes 0-5 - Welcome and workplace integrity dilemma warm-up", '''<p>Welcome students, seat them with devices or the 3-page reference guide, and project the ethical dilemma prompt.</p><ul><li>Ask students: <em>“If an apprentice technician accidentally drops an expensive calibrated torque wrench on the concrete floor, but it looks completely fine on the outside, what happens if that technician stays silent and puts it back in the tool crib without reporting it?”</em></li><li>Collect 2-3 student thoughts: The internal calibration mechanism could be thrown off by 15%, causing wheel lug nuts or aircraft cylinder head bolts to be improperly torqued, potentially leading to catastrophic equipment failure on the highway or in the sky!</li><li>Bridge with, <em>“Workplace ethics is not just being polite; it is a critical safety and operational discipline. Today we analyze real-world workplace dilemmas, distinguish four ethical traits, and audit our own course artifacts for accuracy.”</em></li></ul>''')
                    + flow("#4a9d2f", "Minutes 5-13 - Four Ethical Workplace Traits &amp; the transparent reporting invariant", '''<p>Display the Four Core Ethical Traits on the screen:</p><ul><li>1. <strong>Integrity &amp; Honesty:</strong> Doing what is right even when unsupervised; reporting errors immediately and keeping transparent, accurate records.</li><li>2. <strong>Accountability &amp; Responsibility:</strong> Owning mistakes without making excuses or shifting blame onto teammates.</li><li>3. <strong>Reliability &amp; Punctuality:</strong> Meeting deadlines, honoring commitments, and showing up ready to work consistently.</li><li>4. <strong>Perseverance &amp; Professional Rigor:</strong> Working through complex challenges while respecting safety boundaries—never taking reckless shortcuts to finish faster.</li><li>The Harm Prevention Rule: In technical careers, hiding a mistake is always far more dangerous than making the mistake itself.</li></ul>''')
                    + flow("#1f617a", "Minutes 13-33 - Four workplace ethical scenario analyses sprint", '''<p>Students analyze four authentic scenario cases in their packet (or Canvas):</p><ul><li>Case 1 (Aviation Maintenance): Dropped calibrated tool - Report vs. conceal.</li><li>Case 2 (Drone Inspection): Weather exceeds maximum wind limits - Abort flight vs. risk crash to meet client deadline.</li><li>Case 3 (Automotive Repair): Overtightened drain plug thread - Document and repair vs. hide with silicone sealant.</li><li>Case 4 (Health / Lab Science): Contaminated test tube - Discard and re-run vs. record false negative.</li><li>For each case, record: (1) Core Ethical Trait, (2) Immediate Action, (3) Formal Log / Handoff Record, (4) Harm Prevented.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 21): Verify students name both the immediate action and the official paper/digital record required.<br>• Lap 2 (Minute 28): Confirm students explain the exact human safety harm prevented by transparent reporting.</li></ul>''')
                    + flow("#e3ad19", "Minutes 33-43 - Personal course artifact audit &amp; CCE Six-Weeks Evidence Log Entry 4", '''<p>Students conduct an honest integrity audit of their own 4SW work:</p><ul><li>Review a prior assignment (e.g., Week 2 4-Year Plan or Week 5 Automotive Brief) and identify one area where they corrected an assumption or refined their data.</li><li>Complete Entry 4 in the CCE Six-Weeks Evidence Log from their CCE binder or digital portfolio.</li></ul>''')
                    + flow("#1f617a", "Minutes 43-50 - Submit Integrity Practice Quiz &amp; Day 4 wrap-up", '''<p>Students complete the 6-question Canvas Quiz: 'Integrity and Accurate Records.' Complete Q6 once; transfer Entry 4 during the same block.</p><ul><li><strong>Safe Trim:</strong> Shorten personal audit sharing; fiercely protect the 4 ethical case analyses, harm prevention logging, and Canvas Quiz completion.</li></ul>''')
                ),
                "MONITOR": (
            ...
            5: {
                ...
                "FLOW": (
                    flow("#5a2d91", "Minutes 0-5 - Welcome and growth mindset recovery warm-up", '''<p>Welcome students, explain the private reflection purpose, and project the self-assessment prompt.</p><ul><li>Ask students: <em>“When a professional pilot debriefs a simulator session, or a master technician reviews a repair log, what is the goal of looking back at past mistakes? Is it to feel bad, or to identify the exact adjustment that guarantees success next time?”</em></li><li>Collect 2-3 student thoughts: Self-reflection is how experts identify blind spots, adjust habits, and master complex systems.</li><li>Bridge with, <em>“Today is an opportunity for targeted reflection and evidence recovery. You will synthesize your 4SW learning across four comprehensive evidence domains into a polished private brief.”</em></li></ul>''')
                    + flow("#4a9d2f", "Minutes 5-13 - Deconstructing the 4-part reflection rubric &amp; exemplar modeling", '''<p>Display the Mid-Year Evidence Rubric and review the four reflective jobs:</p><ul><li>Part 1: <strong>Evidence-Based Reasoning &amp; Technical Accuracy:</strong> Explain how separating visible clues from unverified assumptions prevents costly errors in technical careers.</li><li>Part 2: <strong>Transferable Skills Across Career Clusters:</strong> Detail how analytical reasoning and precise communication operate across two distinct occupations.</li><li>Part 3: <strong>Career Organizations &amp; Professional Boundaries:</strong> Contrast the role of regulatory agencies (FAA), credentials (ASE), and school CTSOs (SkillsUSA/TSA).</li><li>Part 4: <strong>Work Ethic, Integrity &amp; S.M.A.R.T. Action Plan:</strong> Document an ethical decision and establish two actionable academic/career goals for the upcoming semester.</li><li>Model Fictional Morgan Evidence Strip: Demonstrate how students with missing earlier work can analyze the supplied evidence strip without penalty.</li></ul>''')
                    + flow("#1f617a", "Minutes 13-38 - Independent Mid-Year Evidence Reflection sprint", '''<p>Students author their 4-Part Evidence Reflection (Canvas Assignment or paper):</p><ul><li>Part 1 (Min 13-19): Evidence vs. Assumption Analysis.</li><li>Part 2 (Min 19-25): Two-Occupation Transferable Skills Demonstration.</li><li>Part 3 (Min 25-31): Professional Organization Classification &amp; CTSO Pathway.</li><li>Part 4 (Min 31-38): Workplace Integrity Reflection &amp; Semester Goals.</li><li><strong>Active Monitoring Checkpoints:</strong><br>• Lap 1 (Minute 20): Check that students cite specific technical examples rather than generic advice.<br>• Lap 2 (Minute 32): Verify students establish concrete, measurable goals for high school preparation.</li></ul>''')
                    + flow("#e3ad19", "Minutes 38-44 - Rubric self-evaluation and targeted refinement", '''<p>Students audit their reflection against the 4-part scoring criteria:</p><ul><li>Self-score each section and execute a 2-minute targeted revision to add concrete evidence to their lowest-scoring part.</li><li>Teacher provides targeted writing support to students with accommodations.</li></ul>''')
                    + flow("#1f617a", "Minutes 44-50 - Submit Private Reflection and 4SW celebration", '''<p>Students submit their completed reflection privately in Canvas or hand in paper packets.</p><ul><li>Celebrate students' dedication and completion of the 4th Six Weeks Career &amp; College Exploration curriculum!</li><li><strong>Safe Trim:</strong> Skip all oral sharing; fiercely protect all four reflective domains, rubric self-score, and private Canvas submission.</li></ul>''')
                ),
