...
def panel_html(day, exit_pdf_id):
    ...
    elif day["response_role"] == "supplemental":
        rows.append(
            f"<li><strong>Optional teacher-selected resource, not a second assignment</strong>: "
            f'<a href="https://docs.google.com/document/d/{day["exit_doc_id"]}/copy">Google Doc (make a copy)</a> · '
            f'<a href="https://docs.google.com/document/d/{day["exit_doc_id"]}/edit">teacher master</a>{exit_pdf}</li>'
        )
    ...
    return (
        ...
        f'<p style="margin:0 0 6px">{"FYF p. 38 is today’s writing space. The Day 1 guide is reference material, not another assignment." if day["response_role"] == "reference" else "FYF holds the required planning or reflection. The Google Doc is optional teacher-selected support; the Canva flyer goes to the Canvas assignment." if day["response_role"] == "supplemental" else "Use the lesson’s workbook or packet response home. The Google Doc here is an alternate, not extra work." if day["response_role"] == "alternate" else "Students see one button per worksheet and one for the exit ticket. The default target is the Google Doc make-a-copy link."} Printable PDFs and the OneNote route are here for teachers who use them.</p>'
