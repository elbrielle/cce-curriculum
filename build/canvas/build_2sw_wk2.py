...
def evidence_images(uploads):
    ...
    return "".join(f'<details style="border:1px solid #cfc5dd;border-radius:8px;padding:12px 16px;margin:12px 0"><summary style="font-weight:700;color:#5a2d91;cursor:pointer">File {n-1}</summary><img loading="lazy" src="/courses/{COURSE_ID}/files/{uploads[f"file-{n-1}-upright.png"]["id"]}/preview" alt="{alts[n]}" style="display:block;width:100%;max-width:680px;height:auto;margin:14px auto;border:1px solid #ddd" data-api-endpoint="/api/v1/courses/{COURSE_ID}/files/{uploads[f"file-{n-1}-upright.png"]["id"]}" data-api-returntype="File"></details>' for n in range(2,8))
...
async def main():
    ...
    async with httpx.AsyncClient(headers={"Authorization":f"Bearer {token}"},timeout=120) as c:
        ...
        for day in range(1,6):
            ...
            await upsert_page_item(c,module_id,teacher,tt); await upsert_page_item(c,module_id,student,st); pages[day]={"teacher":teacher,"student":student}
            if day==2: await upsert_discussion_item(c,module_id,discussion)
        ...
        for day in range(1,6):
            ...
            if day==2:
                practice=next(i for i in items if i.get("type")=="Discussion" and i.get("content_id")==discussion["id"])
