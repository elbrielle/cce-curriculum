...
async def main():
    ...
    async with httpx.AsyncClient(headers={"Authorization":f"Bearer {token}"},timeout=120) as c:
        ...
        teacher_data={
          ...
          5:{"TITLE":"Xello Learning Style Quiz and Learning Styles Lesson","SUBTITLE":"50 minutes - TEKS d(1)(A)","ALERT":"<strong>Protect both Grade 7 tasks.</strong> Learning Style uses 20 minutes and the Learning styles lesson uses 30 minutes. Week 2 supplied the three saved-career dependency. Collect the completed Major 1 packet before students open Xello; do not squeeze the former pitch into this block.","PREP":f'<ul><li>Check Xello rosters, the Completion Standards report, and whether each student has three saved careers from Week 2.</li><li>Open the licensed <a href="/courses/{COURSE_ID}/files/{files["XELLO"]["id"]}/preview">Grade 7 Learning styles guide</a> and the teacher demo account.</li><li>Print the <a href="/courses/{COURSE_ID}/files/{files["REFLECTION"]["id"]}/preview">Method-to-Task Connection</a>; its built-in visual, auditory, and tactile chart is the complete no-Xello content route.</li><li>Post one collection location for the completed Major 1 packet.</li></ul>',"EVIDENCE":"<p><strong>Major grade:</strong> App Design Packet + Emerging Career Evidence, 16 points, collected before the Xello block. Separate Grade 7 evidence is the Learning Style quiz, Learning styles lesson, and one method-to-task connection. The method-chart route completes today's learning evidence when Xello is blocked without creating false platform completion.</p>","FLOW":flow("#5a2d91","Learning Style quiz - 20 minutes","Predict from three method groups, complete the 20 questions, and review the result.")+flow("#1f617a","Learning styles lesson - 30 minutes","Open View all lessons, complete Learning styles, and record one method, task, obstacle, and next test."),"MONITOR":"<p>Lap 1: correct Learning Style quiz. Lap 2: quiz complete and the Learning styles lesson open. Lap 3: the connection names a specific method and task rather than a fixed label. If three saved careers are missing or Xello is blocked, move the student to the method chart and connection sheet and record one supervised catch-up list.</p>","SUPPORT":"<p>Show examples for watching/reading, listening/speaking, and hands-on/movement before naming the three styles. Keep the word bank visual, auditory, tactile, method, obstacle, and strategy beside the connection. Allow oral rehearsal or speech-to-text.</p>","FALLBACK":"<p>The built-in method chart and connection sheet are the complete learning-evidence route for the day. Record the access or three-career dependency need and schedule the Grade 7 quiz and lesson separately. Do not delay the Major 1 submission or mark paper work as Xello completion.</p>"}}
        contracts={
