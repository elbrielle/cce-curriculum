---
six_weeks: "5th Six Weeks"
week: "Week 1"
title: "Blueprint Builders: Architecture Careers"
subtitle: "Architecture & Construction Cluster + TinkerCAD 3D Design"
topic: "Architecture and Design Career Exploration"
length: "5 class periods (45-60 minutes each)"
cluster: "Architecture & Construction"
pathways: "Architecture (MacArthur - AutoDesk Revit Architecture)"
---

## Lesson Objective

Students will explore the Architecture & Construction career cluster through Hats & Ladders, investigate careers in architectural design and pre-construction planning, compare education pathways for architecture careers, and design a building in TinkerCAD to connect 3D modeling skills to real architectural practice.

## Demonstration of Learning

> "I can describe the A&C career cluster, compare education requirements for at least three architecture careers, and design a 3D building model in TinkerCAD."

## TEKS Alignment

- **d(1)(B):** Explore and describe the CTE career clusters.
- **d(1)(C):** Identify various career opportunities within the A&C cluster.
- **d(2)(A):** Research and describe applicable academic, technical, certification, and training requirements.
- **d(5)(E):** Use resources to compare salaries of at least three careers.

## Materials Needed

- Chromebooks with internet access
- Hats & Ladders student accounts
- TinkerCAD: https://www.tinkercad.com
- BLS - Architects: https://www.bls.gov/ooh/architecture-and-engineering/architects.htm
- BLS - Drafters: https://www.bls.gov/ooh/architecture-and-engineering/drafters.htm
- SketchUp for Schools: https://www.sketchup.com/education
- eDynamic Learning Unit 3.1: Planners, Builders, Fabricators
- Printed Architecture Career Research worksheet
- Projector for TinkerCAD teacher modeling

## Career Connection

Architecture is one of the oldest and most creative career paths. Architects design buildings where we live, work, learn, and play. MacArthur High School offers an Architecture pathway leading to AutoDesk Revit Architecture certification. What makes architecture unique is the LONG education pathway: becoming a licensed architect requires a professional degree (5-7 years), an internship, and passing the Architect Registration Examination. This creates a great contrast with the trades careers from the 4th Six Weeks.

**Irving ISD Pathway: Architecture (MacArthur) leads to AutoDesk Revit Architecture certification.**

## Vocabulary

- **Architecture:** The art and science of designing buildings and other physical structures.
- **Blueprint:** A detailed technical drawing of a building, now created digitally using CAD software.
- **CAD (Computer-Aided Design):** Software used to create precision drawings. AutoDesk Revit and AutoCAD are industry standards.
- **Drafter:** A professional who creates detailed technical drawings based on an architect's designs. Requires less education than a licensed architect.
- **Licensed Architect:** An architect who has completed a professional degree, internship hours, and passed the ARE to practice independently.

## Bridge to Theory (Hats & Ladders)

In H&L, the A&C cluster contains the Design/Pre-Construction pathway with Hats like Architect, Drafter, Interior Designer, Urban Planner, and Landscape Architect. Education pathway indicators show significantly longer timelines than trades careers.

## IISD Instructional Strategies

- **Modeling:** Teacher models TinkerCAD on projector before students begin: basic shapes, grouping, alignment, and measurement tools.
- **Sentence Stems:** For salary comparison: 'An Architect earns $_____ after _____ years of education. A Drafter earns $_____ after _____ years.'

## Lesson Sequence

### Day 1: Welcome to Architecture & Construction

**WARM-UP: Look around this room. Someone DESIGNED this building. Who made those decisions?**

#### H&L A&C Cluster Tour (20 min)

> **Teacher:** "We are starting a brand new cluster: Architecture and Construction. Open H&L and navigate to this cluster."

> [H&L PLATFORM] Students explore the Architecture and Construction cluster in H&L. The workbook (Ch 3) lists six pathways: Carpentry, Architecture Drafting and Design, Construction Management and Inspection, Electrical, and others. Key workbook activities: "Safety Supervisor" (workplace safety), "Power Pitch" (elevator pitch), "Trash to Treasure" (repurposing materials), and "Unexpected Architecture" (novelty building design for the city of Brighton Ridge). Students explore pathways in the app for 15 minutes.

#### Architecture Career Profiles (20 min)

> [H&L PLATFORM] Students explore Hats: Architect, Drafter, Interior Designer, Urban Planner, Landscape Architect.

**DOK 2: How would you compare the education timeline for becoming an Architect versus a Drafter?**

**EXIT TICKET: Name two A&C careers and describe the education required for each. (d(1)(B), d(2)(A))**

### Day 2: Architecture Salary Comparison + eDynamic 3.1

**WARM-UP: Guess: How many years of school to become a licensed architect?**

#### Salary and Education Comparison (20 min)

> **Teacher:** "Architecture education is VERY different from automotive. Compare at least 3 architecture careers on salary, education time, and cost. Add one trades career from last six weeks for contrast."

Students complete the Architecture Career Research worksheet comparing Architect, Drafter, Interior Designer plus one trades career.

> [H&L PLATFORM] Students pull DFW salary data from H&L.

**DOK 3: Are the extra years of architecture education 'worth it' compared to a trades career? What factors beyond salary matter?**

#### eDynamic 3.1: Planners, Builders, Fabricators (15 min)

> **Teacher:** "eDynamic Unit 3.1 covers A&C, Manufacturing, and Transportation. Focus on the A&C content since you already explored the other two."

Students begin eDynamic Unit 3.1, focusing on Architecture & Construction sections.

> [VERIFY IN eDynamic] Confirm what specific A&C content is in eDynamic 3.1 and whether students can skip previously explored clusters.

**EXIT TICKET: Compare salaries of 3 architecture careers. Best salary-to-education ratio? (d(5)(E))**

### Day 3: TinkerCAD Introduction: Designing a Building

**WARM-UP: If you could design any building, what would it be?**

#### TinkerCAD Teacher Modeling (15 min)

> **Teacher:** "Real architects use AutoDesk Revit. At MacArthur, you can earn Revit certification. Today we use TinkerCAD, a free 3D tool that teaches the same fundamentals."

Teacher models TinkerCAD: login, workspace, shapes, resizing, grouping, ruler tool.

#### Design Challenge: My Dream Building (25 min)

> **Teacher:** "Design a building with at least 4 walls, a roof, a door, and 2 windows. Think like an architect."

Students begin 3D building designs.

**DOK 2: How do the shapes in TinkerCAD connect to real building materials?**

Engineering Design Process: Define and Ideate -- Students sketch on paper first, then move to TinkerCAD.


**EXIT TICKET: Describe one design decision you made and why. (d(1)(C))**

### Day 4: TinkerCAD: Iterate and Refine

**WARM-UP: What do you want to improve about your building today?**

#### Design Iteration (35 min)

Students iterate. Teacher circulates asking career-connection questions.

**DOK 4: If a client wanted an energy-efficient redesign, what changes would you make? How does this connect to green building careers?**

Engineering Design Process: Prototype and Test -- Students refine buildings: interior rooms, color, landscaping.


**EXIT TICKET: One way your TinkerCAD process is similar to a real architect and one way it differs. (d(1)(C), d(2)(A))**

### Day 5: Building Presentations + A&C Favorites

**WARM-UP: 1-10, how much did you enjoy 3D design? Does architecture interest you?**

#### Design Presentations (20 min)

Students present TinkerCAD buildings: design decisions and career connections. 2 min each.

#### H&L Favorites + eDynamic 3.1 (20 min)

> **Teacher:** "Favorite A&C careers in H&L. Continue eDynamic 3.1 if needed."

> [H&L PLATFORM] Students favorite A&C careers in Climber Profile.

> [VERIFY IN eDynamic] Confirm how much of eDynamic 3.1 to complete this week vs. later.

**EXIT TICKET: Most interesting A&C career and which IISD high school offers it. (d(1)(C))**

## Formative Assessment

- H&L A&C exploration (Day 1). (d(1)(B))
- Salary comparison accuracy (Day 2). (d(5)(E))
- TinkerCAD progress (Days 3-4). (d(1)(C))
- eDynamic 3.1 check (Day 5). (d(1)(B))

## Summative Assessment

TinkerCAD Building Presentation (Day 5): Scored on design quality (d(1)(C)), career connections (d(2)(A)), salary comparison (d(5)(E)).

## Differentiation

### Scaffolded Learning

- TinkerCAD starter template with basic floor plan.
- Simplified salary worksheet with one career pre-filled.
- Pair work option for TinkerCAD.

### Extensions

- Research MacArthur Revit certification pathway + 4-year course plan.
- Add interior rooms and landscaping to TinkerCAD.
- Compare Architect vs. Civil Engineer career trajectories.

### ELL Language Support

- Pre-teach: Architecture = Arquitectura, Blueprint = Plano, Design = Diseno.
- TinkerCAD is visual and accessible. Bilingual command card.
- Bilingual salary worksheet.
- Pair ESL with bilingual peers for presentations.
