...
- Added a research-grounded worksheet design standard and linked it from the Canvas lesson workflow. The gate now requires a thinking-job-first layout, worked or partially worked models, graduated scaffolds, point-of-use language support, response areas sized from the real writing task, strict rendering, and visual inspection.
- Rebuilt the Nursing Route Comparison as a three-page `See one -> Try one -> Compare -> Decide` organizer. Students use a completed Nursing Assistant model, a guided LVN evidence bank, aligned RN/NP route cards, and a fictional-student recommendation instead of copying four careers into a cramped table.
- Rebuilt and visually inspected all four changed printables: the three-page route comparison, two-page route guide, two-page simulator build, and two-page clinical handoff record. All passed strict page-count and overflow checks with writing space matched to the requested response.
...
- Reconciled unpublished module 542989 to seventeen ordered items: five Day subheaders, five teacher guides, five student guides, the existing five-question practice Quiz, and the existing Minor assignment. All pages, items, interactives, and the module remain unpublished; licensed-resource folders remain locked.
- Module-specific API QA returned no problems. Signed-in Canvas review confirmed the daily contracts, chronological module route, preserved Quiz and Minor submission routes, HQIM/source distinction, and no horizontal page overflow at the available desktop and 960-pixel browser widths. Local PDF rendering and the Canvas file records were used to verify the downloadable artifacts because the browser's PDF preview extension blocked the Canvas CDN preview.
