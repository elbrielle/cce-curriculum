# Week 3: Built to Last — Construction Evidence

**5th Six Weeks** | Architecture and Construction Cluster | 5 class periods (50 minutes each)

## Lesson Objective

Students compare construction careers and preparation routes, investigate the steps for a Registered Apprenticeship and a Dallas College construction program, classify construction occupations with one published course rule, and turn visible clues from a fictional property into a careful evidence report and short professional briefing.

**Learning Target:** I can use dated evidence to compare construction careers and routes, explain what an assessment image supports and does not support, and communicate one finding clearly.

**Success Criteria:** My work keeps source labels with every number, records actual apply/enroll steps, classifies careers under the published rule, separates observation from inference, and names the qualified role that should evaluate a possible concern.

## TEKS Alignment

- **d(1)(C):** Identify construction career opportunities and the work products or decisions each role owns (Days 1, 4, 5).
- **d(2)(A):** Research and compare documented preparation requirements (Day 1).
- **d(3)(G):** Investigate and report the steps required to apply to a Registered Apprenticeship and enroll in a community or technical college route (Day 2).
- **d(3)(H):** Distinguish a union, professional/trade association, CTSO, and credential body; explain the value of one affiliated organization (Day 2).
- **d(5)(A), d(5)(B):** Analyze labor trends and classify occupations with a consistent, published evidence rule (Day 3).
- **d(4)(C):** Deliver an individual 30–45 second professional evidence briefing (Day 5).

## Evidence Boundary

The home-inspection sequence is a **fictional visual observation and report-writing exercise**. Students do not inspect their homes, touch wiring, climb roofs, handle mold, use tools, operate a thermal camera, diagnose a defect, estimate repair cost, recommend a purchase, or give repair instructions. A strong claim uses this structure:

1. **I observe:** only what is visible.
2. **This could indicate:** one cautious possibility.
3. **Evidence limit:** what the image cannot prove.
4. **Next qualified role:** who should evaluate the concern.

Texas Professional Real Estate Inspectors are licensed professionals. The Grade 8 activity does not imitate or replace a licensed inspection.

## Current Local and Source Decisions

- The current public MacArthur page names the pathway **Construction** within the School of Architecture, Construction and Civil Engineering (ACE).
- The 2026–27 Irving ISD coursebook lists Principles of Construction, Construction I, Construction II, and Practicum in Construction; Autodesk Associate (Certified User) Revit Architecture; and Technology Student Association (TSA).
- The FYF district pages use older **Construction Technology**, NCCER Core, and SkillsUSA wording. Present that only as a dated workbook snapshot, not a current promise.
- H&L is optional exploration. It is not the scored salary source and no favorite/rating is required.
- No required Grade 8 Xello or eDynamic task is assigned this week.

## Course Classification Rule

This is a transparent classroom comparison rule, not an official government designation. All scored rows use May 2024 U.S. BLS medians and 2024–34 national projections.

- **High-skill:** the BLS preparation field documents substantial occupation-specific preparation such as apprenticeship, moderate/long-term on-the-job training, a bachelor’s degree, or a regulated license/credential boundary.
- **High-wage:** May 2024 U.S. median is above the all-occupation median of **$49,500**.
- **High-demand for this comparison:** projected 2024–34 growth is above the all-occupation rate of **3.1%**.

Annual openings are analyzed separately. Openings include replacement needs and do not by themselves prove a local labor shortage.

## Week at a Glance

| Day | Focus | Individual evidence | Primary TEKS |
|---|---|---|---|
| 1 | Current pathway + career preparation | Career/preparation evidence card | d(1)(C), d(2)(A) |
| 2 | Registered Apprenticeship, college route, and organizations | Route/enrollment and organization comparison | d(3)(G), d(3)(H) |
| 3 | Fixed labor-market classification | Four-career classification and limitation | d(5)(A), d(5)(B) |
| 4 | Spot the Problem observation lab | Five-image observation log | d(1)(C) |
| 5 | Evidence report + professional briefing | Private report and individual oral/text equivalent | d(1)(C), d(4)(C) |

## Materials

- Chromebooks and Canvas
- Locked FYF pp. 176–179 and locked Climber Notes slides 2–6
- Fixed CCE career, route, organization, and labor-data cards
- Career evidence card, route comparison, classification packet, observation log, report, and student-visible rubric
- Optional H&L launch; no app completion evidence

## Assessment and Grading

- Days 1–2: formative.
- Day 3: **Minor 3**, the individual labor-evidence classification.
- Day 4: formative observation log with a full teacher key.
- Day 5: formative individual inspection-evidence report plus 30–45 second live, private recorded, or teacher-conference briefing under one rubric.

The [5SW assessment map](../../resources/six-weeks-assessment-map.md) is the placement authority. Keep graded Canvas items unpublished until the Minor/Major groups and 40%/60% weighting are verified. Team attendance, public-speaking confidence, grammar, drawing, accent, H&L state, and platform access do not determine mastery.

## Current Sources

- [BLS Carpenters](https://www.bls.gov/ooh/construction-and-extraction/carpenters.htm)
- [BLS Construction Managers](https://www.bls.gov/ooh/management/construction-managers.htm)
- [Apprenticeship.gov Career Seekers](https://www.apprenticeship.gov/career-seekers)
- [Dallas College Construction Technology](https://www.dallascollege.edu/study/construction-technology/)
- [Dallas College application steps](https://www.dallascollege.edu/admissions/apply-texas/)
- [TREC Professional Real Estate Inspector](https://www.trec.texas.gov/become-licensed/professional-real-estate-inspector)
- [NCCER credentials and certifications](https://www.nccer.org/credentials-certifications/)

## Access and Absence Route

Every required source appears in the locked Canvas module with adjacent text. Students may type, annotate, dictate, record privately, or use paper. The five inspection images have answer-neutral descriptions, zoomable files, and a teacher read-aloud route. Absent students complete the same individual evidence; no partner, team, live presentation, H&L login, or real property is required.
