# Day 5: Personality Style, Three Careers, and IT Decision
<!-- CCE DAILY CONTRACT START -->
## Daily Learning Contract

- **Topic:** Career Fit
- **Objective:** Students will analyze one Personality Style result and use career evidence to make an Information Technology fit decision.
- **TEKS:** d(1)(A), d(1)(C), d(5)(A), d(5)(E)
- **Demonstration of Learning:** Five-page IT Salary Comparison and Career Fit Reflection packet submitted as Minor 2; when Xello is accessible, Personality Style is complete and three careers are saved to unlock next week's Grade 7 Learning styles lesson.
<!-- CCE DAILY CONTRACT END -->

## Lesson Overview

| | |
|---|---|
| **Time** | 50 minutes |
| **Objectives** | Complete the Grade 7 Xello Personality Style quiz; connect one trait to a real example; save three researched careers for next week's lesson; use the week's salary evidence to make an IT fit decision |
| **TEKS** | d(1)(A), d(1)(C), d(5)(A), d(5)(E) |
| **5E Phases** | Engage: assessment launch · Explore: Personality Style and career saving · Explain: Career Fit Reflection · Evaluate: IT decision |
| **Deliverable** | IT Salary Comparison and Career Fit Reflection; Xello Personality Style and three saved careers are checked separately when accessible |
| **Materials** | 1 Chromebook per student, district SSO, Xello, each student's returned five-page salary packet, and [Minor 2 rubric](../../resources/worksheets/wk2-salary-hoc-rubric.pdf). The [one-page IT Pathway Decision](../../resources/worksheets/wk2-day5-it-pathway-decision.pdf) is a lost-packet, enlarged-print, or catch-up route, not a second whole-class assignment. |

## Before Students Arrive

1. Check the Xello Completion Standards report. Personality Style requires Matchmaker, so list students who need the teacher-sample result and written evidence route instead of discovering the dependency during class.
2. Open the licensed Xello Personality Style teacher resource in the locked Canvas teacher guide.
3. Return each student's five-page salary packet. Display the rubric. Print the one-page decision only for students using the replacement route.
4. Ask students to mark three careers from their completed packet that they can search and save after the quiz. Saving three careers is prerequisite support for next week's Grade 7 lesson, not a separate completion standard or grade.
5. Keep Xello results and reflection evidence individual and private. An adjacent peer may help with navigation without seeing or answering the assessment.

## Launch and Prerequisite Route (5 min)

Students open Xello Personality Style. Anyone whose Matchmaker dependency is missing moves to page 5 of the salary packet with the teacher-supplied sample result while the teacher adds the platform task to one supervised catch-up list. Do not let multiple students lose the first ten minutes trying random menus or creating another account.

## Activity 1: Xello Personality Style (20 min)

Students open **ClassLink > Xello > About Me > Personality Style**, complete the quiz, and review the result. They privately record:

- one trait that sounds like them; and
- one example from school, home, a team, or an activity that supports or challenges that result.

Remind students that an assessment suggests patterns; it does not decide who they are or which career they must choose. Check completion through the teacher report. Do not require profile screenshots.

**Active monitoring:** First lap checks that students reached Personality Style rather than another quiz. Second lap checks completion progress. Third lap asks students to explain one trait in their own words. If Matchmaker is missing, move the student to the written reflection and add the platform task to supervised catch-up.

**CFU and privacy:** Ask students to point to the trait label and the example box without sharing the result aloud. If more than four students lack the prerequisite or Xello fails, move those students to page 5 and use one catch-up roster rather than troubleshooting accounts one at a time.

## Activity 2: Save Three Researched Careers (20 min)

Students use the three careers marked in their salary packet. They open **Xello > Explore Careers**, search each exact career, review enough of the profile to confirm the title, and save all three.

This is prerequisite support for next week's Grade 7 Learning styles lesson. It is not a Grade 7 completion standard, a separate assignment, or evidence that replaces the salary packet. Students do not search for three new careers or copy profile text.

## Activity 3: Minor 2 Career Fit Reflection and Submission (5 min)

Students finish the four labeled jobs on page 5 of the returned salary packet. Use the [IT Pathway Decision](../../resources/worksheets/wk2-day5-it-pathway-decision.pdf) only when the original packet is unavailable or a one-page enlarged route is needed:

1. Name the IT pathway they rated.
2. Cite one salary or labor-market fact from the completed three-career comparison.
3. Name one career they reviewed and explain why it fits or does not fit.
4. Decide whether they could see themselves in IT and support the decision with one Personality Style result, the teacher-supplied sample result, or a programming experience.

Score the combined salary comparison and reflection with the [Minor 2 rubric](../../resources/worksheets/wk2-salary-hoc-rubric.pdf). Code.org completion, H&L favorites, and the daily exit are not separate grades.

Keep these words beside page 5: **trait, example, evidence, preparation, task, fits, does not fit, might fit**. A complete response frame is: "My result included _____. This career fits / does not fit / might fit because the work requires _____, and my evidence is _____."

Students check that pages 1-4 hold three career records and one comparison, then check all four page 5 reflection jobs. They submit all five pages through the named Minor 2 route and confirm that any platform catch-up need is on the teacher's list. Students may reject the IT cluster and still demonstrate full mastery when the evidence and reasoning are specific.

## Answer Guidance and Monitoring

There is no single correct career-fit decision. Full-credit work accurately represents the student's assessment result and the cited app evidence. Do not score English mechanics unless meaning is unclear. Redirect claims such as "the quiz says I must be a programmer" or salary numbers with no career/geography/measure attached.

## Supports and Fallbacks

- Read result descriptions aloud, provide trait/example sentence frames, and allow oral rehearsal or speech-to-text.
- Use bilingual field labels and a short vocabulary bank rather than translating the entire assessment.
- A student may keep a personal result private and discuss only the trait they choose to use as evidence.
- If Xello is unavailable or Matchmaker is incomplete, record the issue and complete page 5 with the teacher-supplied sample result and the student's existing career evidence. The Minor 2 route is complete without waiting for the platform. Schedule Personality Style and three-career saving for supervised catch-up; written work does not create false Xello completion.
- Code.org may continue only as an early-finisher extension after required work is complete.
